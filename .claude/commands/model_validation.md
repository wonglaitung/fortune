...
| 模型类型 | 目标 | 标签类型 | 验证脚本 | 数据泄漏阈值 | 特征数量 |
|----------|------|----------|----------|--------------|----------|
| **恒指模型** | 择时（预测大盘涨跌） | **绝对标签** | `hsi_walk_forward.py` | >80% 可疑 | 33个（增强模型） |
| **个股模型** | 选股（挑出跑赢的股票） | **相对标签** | `walk_forward_validation.py` | >65% 可疑 | 730个（完整模型） |

**核心原则**：
- **个股模型必须用相对标签**：目标是选股，判断"跑赢/跑输"其他股票
- **恒指模型必须用绝对标签**：目标是择时，判断"涨/跌"
- **禁止混用**：个股用绝对标签会导致"全涨全跌"，恒指用相对标签无意义
