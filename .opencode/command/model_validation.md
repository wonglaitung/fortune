...
| 指标 | 正常范围 | 优秀 | 数据泄漏信号 |
|------|---------|------|-------------|
| 20天准确率 | 54-58%（PIT 实测） | >58% | **>85%** |
| 5天准确率 | 53-56%（PIT 实测） | >56% | >70% |
| 1天准确率 | ~50-51%（PIT 实测） | - | >60% |

> ⚠️ 恒指旧"正常 60-80%"由泄漏贡献；PIT/embargo 实测 20d≈57.5% 且与随机不可区分。
> 通过与否以**阶段 5.5（lift/方向技能）+ 5.6（月度护栏）**为准，**不以准确率**。
...
|------|-----------------|--------------|-------------|
| 准确率 | 50-55% | 50-55% | **>65%** |
| 净IR（月度护栏口径，见阶段 5.6） | 0~1.5 | 0~1.5 | - |
| 最大回撤 | 视行情 | 视行情 | - |
| 超额 lift / 方向技能 | >0 且显著为准 | >0 且显著为准 | - |
| 总体盈亏比 | 仅参考 | 仅参考 | - |

> ⚠️ **旧口径作废**：2026-05 的"夏普 5.0-6.0 / 收益率 +5%~+6%"由静态快照穿越贡献（PIT 修正后大幅下修）。
> 通过与否以 **阶段 5.5（lift/方向技能）+ 阶段 5.6（月度护栏净IR/PBO/DSR）** 为准（见 `docs/DECISIONS.md`）。
...
| 对比项 | 恒指 | 个股 | 说明 |
|--------|------|------|------|
| 准确率上限 | 57.5%（PIT） | 51.6%（PIT） | 均接近随机 |
| 预测概率与实际方向相关性 | 弱（r≈+0.16） | 弱（r≈+0.04） | 均较弱 |
| 最优策略 | 三周期模式均接近随机，不构成信号 | 三周期模式均接近随机 | 不能套用旧"假突破95%" |
...
| 策略代码 | 策略名称 | 20天准确率 | 同向20d基准净贡献 | p值 |
|---------|---------|-----------|------------------|-----|
| 000 | 一致看跌 | 65.45% | +7.6pp | 0.034 |
| 111 | 一致看涨 | 67.91% | +7.6pp | 0.078 |
| 001 | 下跌中继 | 55.45% | −4.9pp | 0.330 |
| 010 | 反弹失败 | 57.14% | −0.7pp | 1.000 |
| 101 | 假突破 | 58.70% | −1.6pp | 0.881 |

> ⚠️ **准确率必须相对「同向 20d 无条件基准」（UP 60.3% / DOWN 57.8%）读**；
> 8 模式 Bonferroni（α=0.00625）后**全部不显著** → 不构成可靠交易信号。
> 旧值（010=58.33/000=53.70/101=32.26 等）系无基准口径 + 样本数自相矛盾（885≠697），已废弃。
...
| 对比项 | 恒指 | 个股 | 说明 |
|--------|------|------|------|
| 预测概率与实际方向相关性 | r≈+0.16 | r≈+0.04 | 均较弱 |
| 20天准确率 | 57.5% | 51.6% | 均接近随机 |
| 三周期模式 | 30%–58% | 53%–57% | 均接近随机 |

**关键教训**：
- 恒指三周期模式经 PIT/embargo 后均接近随机（101 假突破仅 32%），**旧"假突破 95%"作废**
...
```markdown
**恒指增强模型**（2026-09-26 复测，PIT/embargo 流程）：

| 周期 | 准确率 | 95%CI | vs 随机 | 超额 lift |
|------|--------|-------|---------|----------|
...
> 上轮（2026-09-23）54.1/55.9/57.5 与 1d "better (p=0.03)" **本轮未复现** → 每次复测必须重跑
> `backtest_eval.py` 与 `monthly_guardrail.py`，不得沿用旧值。
```
...
### 阶段 5.5：严谨合并评估（backtest_eval.py，必须执行）⭐

**目的**：用业界口径复核模型，**替代只看绝对准确率/胜率**。
...
```bash
# 个股模型（周期必须与 Walk-forward 一致）
python3 ml_services/backtest_eval.py \
  --input output/<最新目录>/prediction_analysis.csv --horizon 20 \
  --compare output/<对比周期目录>/prediction_analysis.csv --compare-horizon 5 \
  --output output/backtest_eval_20d.md
```

**参数**：`--horizon` 必须等于本次 Walk-forward 周期；`--cost` 默认 0.005（与 `TOTAL_COST` 一致）；
`--compare/--compare-horizon` 可选，用于跨周期一致性标注。

#### 5.5B. 必看指标（判读）

| 章节 | 指标 | 判读 |
|------|------|------|
| 二 | **超额 lift** = 信号胜率 − 无条件买入基准 | >0 才有选股/择时能力 |
| 三 | **方向技能** = 准确率 − "永远看涨" | >0 才说明方向判断超越趋势 |
| 六 | 逐股票（名称 + 跨月一致性） | 只看全期样本足 + 跨周期/跨月一致者 |
| 七 | 逐月表现 | 月度波动极大，看 lift 分布而非单月 |
| 八 | 月份×股票 | ⚠️ **多重比较噪声**（n_eff≈4），禁用 |

**典型陷阱**：银行等防御板块绝对准确率高（如 58.3%），但"永远看涨"基准 62.4% →
方向技能 −4.2pp、lift +0.9pp，属 **beta 陷阱**，不是能力。

#### 5.5C. 阶段 5.5 检查清单

- [ ] 已对最新 `prediction_analysis.csv` 运行 `backtest_eval.py`
- [ ] 已记录**超额 lift 与方向技能**（不是绝对准确率/胜率）
- [ ] 已用 `--compare` 检查跨周期一致性
...
- [ ] 已对最新 `prediction_analysis.csv` 运行 `backtest_eval.py`
- [ ] 已记录**超额 lift**（胜率 − 基准）与**方向技能**（准确率 − 永远看涨）
...
3. **验证方法**
   - 检查特征工程代码中的 `.shift()` 调用
   - 确认训练数据和预测数据的预处理一致
   - 运行 `backtest_eval.py` 看 **lift / 方向技能**（绝对准确率会骗人）

4. **"假显著"陷阱**
   - 1d 准确率 p=0.0008 看似显著，但 IC 仅 0.017、多空毛收益 0.09%/期 → 经济价值为零
   - "月份×股票"极值（90–100%）是多重比较的必然结果，n_eff≈4，不可用
   - 绝对胜率受行情主导，必须减"无条件买入"基准
...
| 数据泄漏阈值 | >85% | >65% |
| 最优策略 | 三周期模式均接近随机 | 三周期模式均接近随机 |
| 预测概率与实际方向相关性 | 弱 r≈+0.16 | 弱 r≈+0.04 |
