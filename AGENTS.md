...
- 📊 **筹码分布分析**：基于成交量的简单分箱法，计算筹码集中度、拉升阻力，集成到主力资金追踪和综合分析系统（2026-03-13新增）
...
> - **CatBoost单模型**：年化收益率 79.54%，夏普比率 1.14，买入信号胜率 30.96%，优秀股票 20/28（71%）
> - 融合模型（加权平均）：年化收益率 3.16%，夏普比率 0.32，买入信号胜率 22.75%，优秀股票 2/28（7%）
>
> **根本原因：信号稀释问题**
...
### 胜率
- **定义**：买入信号胜率 = 盈利交易数 / 买入信号总数
- **含义**：在所有买入信号中，最终盈利的比例
- **作用**：评估买入信号的实际盈利能力
- **示例**：如果买入信号胜率为23.04%，说明每100次买入只有约23次盈利
- **注意事项**：胜率只考虑买入的交易，不包括"不买入"的正确决策

### 准确率
- **定义**：准确率 = 预测正确的交易数 / 总交易机会数
- **含义**：模型预测方向正确的比例（包括正确预测上涨和正确预测下跌）
- **作用**：评估模型的方向预测能力
- **示例**：如果准确率为86.21%，说明在573个交易机会中，有494个预测正确
- **注意事项**：高准确率不一定意味着高收益（参见2024年10月案例分析）

### 正确决策比例
- **定义**：正确决策比例 = (盈利买入数 + 正确不买入数) / 总决策数
- **含义**：所有决策（买入或不买入）中，正确决策的比例
- **作用**：综合评估模型的决策质量
- **示例**：如果正确决策比例为75.22%，说明模型的正确决策占大部分
- **注意事项**：这是最全面的评估指标，包含了买入和不买入两种决策的正确性
...
8. **代码注释规范** - 注释应该描述功能，而不是变更历史
   - **不要添加"新增"、"修改"、"删除"等字眼**：这些标记会干扰大模型理解和代码维护
   - **变更历史由 git 管理**：使用 `git log` 和 `git diff` 查看变更历史
   - **大模型提示词规范**：提示词应该专业规范，平等对待所有技术指标，避免干扰性标记
   - **正确做法**：注释应该简洁明了，描述代码的功能和目的
   - **错误做法**：`# 计算筹码分布（新增）` → `# 计算筹码分布`

9. **README 内容规范** - README 应该保持简洁和时效性，避免过时的日期标注
   - **不要添加日期标注**：避免在 README 中添加"（2026-03-13新增）"等反映更新状态的标注
   - **保持内容简洁**：README 应该面向当前用户，展示当前功能和状态
   - **版本历史由 git 管理**：使用 `git log` 查看功能变更历史
   - **正确做法**：直接描述功能，如"筹码分布分析：基于成交量的简单分箱法"
   - **错误做法**："筹码分布分析（2026-03-13新增）：基于成交量的简单分箱法"
...
│   │   ├── **筹码分布分析**（get_chip_distribution）⭐ 新增（2026-03-13）
│   │   │   ├── 基于成交量的简单分箱法
│   │   │   ├── 筹码集中度计算（HHI指数）
│   │   │   ├── 上方筹码比例（拉升阻力分析）
│   │   │   ├── 筹码集中区识别
...
│       │   ├── 按准确率排名
│       │   └── 综合优秀股票识别
│       ├── **月度趋势分析** (backtest_monthly_analysis.py) ⭐ 新增（2026-03-06）
│       │   ├── 2024-2026年跨年度回测月度分析
│       │   ├── 月度收益率、正确决策比例、准确率统计
│       │   ├── 季节性趋势识别
│       │   └── 表现最佳/最差月份识别
│       ├── **股票月度趋势对比** (stock_monthly_trend_analysis.py) ⭐ 新增（2026-03-06）
│       │   ├── 单个股票与总体趋势相关性分析
│       │   ├── 波动性分析（标准差、变异系数、最大回撤）
│       │   ├── 稳定性分析
│       │   └── 月度异常值检测
...
- 集成新闻分析和板块分析数据
- **筹码分布分析集成**（2026-03-13新增）：为每只股票计算筹码分布，识别拉升阻力
...
### 模型性能（2026-03-06 最新，来自 model_accuracy.json）
...
#### 2024-2026年跨年度回测详细表现（置信度0.55，28只股票，2026-03-06新增）
- 总交易机会：13,457个
...
- 添加"阻力标识"栏位（2026-03-13新增）：基于筹码分布分析，标注拉升阻力
  - ✅：低阻力（上方筹码 < 30%），拉升容易
  - ⚠️：中等阻力（30-60%），注意风险
  - 🔴：高阻力（> 60%），拉升困难
  - N/A：无法计算（数据不足）
...
# 月度趋势分析（2024-2026年跨年度）⭐ 新增（2026-03-06）
python3 ml_services/backtest_monthly_analysis.py

# 股票月度趋势对比分析 ⭐ 新增（2026-03-06）
python3 ml_services/stock_monthly_trend_analysis.py
...
**关键发现**：
1. **模型表现卓越**：准确率和正确决策比例均超过82%，远超随机水平
2. **收益率可观**：平均收益率8.76%，年化收益率远超市场基准
3. **风险可控**：夏普比率2.43，风险调整后收益优秀
4. **市场环境敏感**：模型在强上涨市场中表现优异

**优秀股票特征**：
- 汇丰银行（0005.HK）：正确决策比例75.22%，准确率92.04%，双料冠军
- 药明生物（2269.HK）：正确决策比例69.03%，准确率90.71%，表现稳定
...
### 2024-2026年跨年度回测综合评估（2026-03-06新增）

**回测概览**：
- 时间范围：2024-01-02 至 2026-01-02
- 股票数量：28只
- 总交易机会：13,457个
- 买入信号：7,554个（占比56.12%）
...
**关键发现**：
1. **收益率呈下降趋势**：上半年（3.79%）> 下半年（2.74%）
2. **季节性明显**：表现最好的月份（2025-01，16.58%）和最差的月份（2025-03，-7.78%）差距巨大
3. **高相关性股票表现一致**：盈富基金、腾讯控股等与大盘高度相关的股票跟随市场趋势
4. **低相关性股票独立性强**：绿色动力环保等低相关性股票表现出独立的走势

**股票分类特征**：
- **高相关性股票（相关系数 > 0.7）**：10只，包括盈富基金、腾讯控股、阿里巴巴等
- **低相关性股票（相关系数 < 0.3）**：4只，包括恒基地产、中国神华、中国海洋石油、绿色动力环保
- **高波动性股票**：地平线机器人、比亚迪股份、华虹半导体等
- **高稳定性股票**：中国移动、中国海洋石油、中国神华等

**投资建议**：
- ✅ 推荐配置高相关性股票进行市场跟踪
- ✅ 适量配置低相关性股票进行风险分散
- ⚠️ 高波动性股票需要严格控制仓位
- ⚠️ 关注季节性规律，在表现好的月份增加仓位
...
- ✅ **筹码分布分析**：完整，基于成交量的简单分箱法，计算筹码集中度和拉升阻力，集成到主力资金追踪和综合分析系统（2026-03-13新增）
...
---
最后更新：2026-03-06（2024-2026年跨年度回测月度趋势分析、新增backtest_monthly_analysis.py和stock_monthly_trend_analysis.py脚本、股票相关性分析、波动性分析、季节性趋势识别）