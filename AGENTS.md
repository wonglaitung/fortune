...
10. **会话状态管理** - 保持会话间的上下文连续性
   - **待办事项记录位置**：`progress.txt`（项目根目录）
   - **会话开始时**：读取 `progress.txt` 了解当前进度和待完成任务列表
   - **完成功能后**：更新 `progress.txt` 记录完成的工作和下一步计划
   - **文件格式**：progress.txt 使用 Markdown 格式，包含已完成工作和待处理事项

11. **经验教训管理** - 持续学习和改进
   - **经验教训记录位置**：`lessons.md`（项目根目录）
   - **会话开始时**：审查 `lessons.md` 了解历史错误和最佳实践
   - **每次纠正后**：更新 `lessons.md` 记录新的经验教训
   - **避免重复错误**：通过 lessons.md 确保不再犯相同错误
