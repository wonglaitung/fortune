...
> **🔴 收益率指标定义必须明确，避免报告误导**
>
> **Walk-forward验证中的收益率指标定义**：
>
> **20天持有期收益率（平均收益率）**：
> - **定义**：每次交易持有20天的收益率
> - **计算**：所有交易机会收益率的算术平均值
> - **表示**：例如 0.52% 表示每20天平均收益 0.52%
> - **用途**：与预测周期一致，直观反映短期收益
>
> **年化收益率**：
> - **定义**：将20天收益率转换为年化收益率
> - **计算**：年化收益率 = 平均收益率 × (252 / 20) = 平均收益率 × 12.6
> - **表示**：例如 0.52% × 12.6 = 6.55%
> - **用途**：便于与年化无风险利率比较，评估长期收益能力
>
> **夏普比率计算**：
> - **必须使用年化收益率**：夏普比率 = (年化收益率 - 无风险利率) / 年化标准差
> - **错误做法**：使用20天收益率计算夏普比率会导致错误结论
> - **正确做法**：先计算年化收益率和年化标准差，再计算夏普比率
>
> **报告要求**：
> - ✅ 必须同时显示20天收益率和年化收益率
> - ✅ 明确说明收益率是20天持有期收益率
...
> - **消费股**：平均收益率 0.52%，年化收益率 6.55%，夏普比率 0.2468
>   - 错误解读："收益率0.52%很低"（未说明是20天收益率）
>   - 正确解读："20天收益率0.52%，年化收益率6.55%，夏普比率最高，风险调整后收益最优"
>
> **建议措施**：
> - **所有Walk-forward验证报告**必须同时显示20天收益率和年化收益率
> - **在报告开头明确说明**收益率指标的定义和转换公式
> - **夏普比率说明**中明确使用年化收益率计算
> - **对比分析时**使用相同的时间周期（都使用20天或都使用年化）
>
> **代码参考**（walk_forward_by_sector.py）：
> ```python
> # 20天持有期收益率
