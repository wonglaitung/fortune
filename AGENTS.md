...
>
> **问题描述**：
> - CatBoost 1天模型准确率63.09%（±4.33%）
> - 标准偏差±4.33%过高，表明模型在不同fold上表现不稳定
> - 准确率远高于其他模型的1天准确率（~53%）
> - 准确率甚至高于CatBoost 5天（63.96%）和20天（61.88%），违反一般规律
>
> **根本原因**：
...
### 深度学习模型警告（2026-03-04）
> **🔴 深度学习模型（LSTM、Transformer）表现远不如CatBoost，不适合用于金融时序预测**
>
> **实验结果**（2026-03-04，1天预测，3只股票平均）：
> - **CatBoost**：准确率 63.09%，F1分数 0.6022，年化收益率 79.54% ⭐⭐⭐⭐⭐
> - **LSTM**：准确率 51.79%，F1分数 **0.0000**，回测无交易 ❌
> - **Transformer**：准确率 51.15%，F1分数 0.1303，回测无交易 ❌
>
> **根本原因**：
> - **数据量不足**：单个股票约700个样本，深度学习需要>10,000个样本
> - **信噪比极低**：股价数据信噪比<10%，深度学习容易学习噪声
> - **特征工程不足**：深度学习主要使用原始价格序列，CatBoost使用500个精选特征
> - **序列依赖假设错误**：股价主要是随机游走，有效市场假说
> - **过拟合风险高**：参数/样本比>140:1（严重过拟合）
>
> **关键发现**：
> - **LSTM模型失效**：F1分数为0，完全无法识别上涨信号（精确率0，召回率0）
> - **预测概率偏低**：所有预测概率都在0.48-0.49之间（接近随机）
> - **回测无交易**：由于置信度阈值55%，没有触发任何交易
> - **无法实际使用**：虽然准确率51.79%，但F1分数0表明模型失效
>
> **验证结果**：
> - ✅ 代码实现正确（LSTM、Transformer架构正确）
> - ✅ 数据处理正确（特征工程、数据分割正确）
> - ❌ 深度学习模型不适合短期股价预测
>
> **建议措施**：
...
>
> **模型可信度评估**：
> - CatBoost 单模型：⭐⭐⭐⭐⭐（高可信度，强烈推荐）
> - LSTM：⭐（低可信度，不推荐）
> - Transformer：⭐（低可信度，不推荐）
...
### CatBoost 单模型策略（2026-03-04 更新）

**为什么选择 CatBoost 单模型？**

经过严格的回测验证和深度学习模型对比实验，CatBoost 单模型表现远优于所有其他模型：

| 模型类型 | 准确率 | F1分数 | 年化收益率 | 夏普比率 | 胜率 | 优秀股票占比 |
|---------|--------|--------|-----------|---------|------|-------------|
...
### 模型性能（2026-03-04 最新，来自 model_accuracy.json）
...
14. **## 十三、数据来源**（11个数据源说明）
15. **## 十四、深度学习模型对比实验**（新增深度学习模型与传统模型对比信息）
...
### 深度学习模型失败分析（2026-03-04）

**实验结果**（1天预测，3只股票平均）：

| 模型 | 准确率 | F1分数 | 预测概率范围 | 回测收益率 | 问题 |
|------|--------|--------|-------------|-----------|------|
| **CatBoost** | **63.09%** | **0.6022** | 0.48-0.75 | **79.54%** | 无 |
| LSTM | 51.79% | **0.0000** | 0.48-0.49 | 0.00% | F1分数为0 |
| Transformer | 51.15% | 0.1303 | 0.47-0.51 | 0.00% | 概率偏低 |
...
5. **过拟合风险高**
   ...
   - CatBoost 参数/样本比：约 7:1（相对健康）

**LSTM模型具体问题**：

- **预测概率特征**：所有股票的预测概率都在 0.48-0.49 之间
...
- **F1分数很低**：0.1303（仅在 0939.HK 有微小表现）

- **回测无交易**：同样无法触发任何交易

- **训练时间长**：比 CatBoost 慢 2-3 倍
...
- CatBoost 是最佳选择：准确率最高、F1分数优秀、实际回测表现优异、训练速度快
- 深度学习模型不适合短期预测：LSTM和Transformer都表现很差，无法实际使用
- 根本原因：金融时序预测的本质问题（信噪比极低、数据量不足、股价随机游走）
...
- `DEEP_LEARNING_COMPARISON_README.md`：深度学习模型对比实验指南（2026-03-04新增）
