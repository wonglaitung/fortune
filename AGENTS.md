...
| 训练 CatBoost 模型 (20天) | `python3 ml_services/ml_trading_model.py --mode train --horizon 20 --model-type catboost` |
...
- ⭐ **全量特征优化**（2026-03-27）：使用全量特征（892个）优于500特征，年化收益率+10.14%，索提诺比率+83%
...
│       │   ├── **特征选择模块**（feature_selection.py）⚠️ 已废弃
│       │   │   ├── F-test特征选择（统计显著性）
│       │   │   ├── 互信息特征选择（关联强度）
│       │   │   ├── 模型重要性法（基于特征对模型预测的贡献）
│       │   │   ├── 混合方法（交集+综合得分）
│       │   │   ├── 统计方法（F-test+互信息混合，已废弃）
│       │   │   └── 统一策略：CatBoost 使用全量特征（892个）
│       │   ├── 分类特征编码（LabelEncoder）
│       │   ├── **超增强正则化**
...
│   ├── 训练工具 (train_with_feature_selection.py) ⚠️ 已废弃
...
3. **特征工程不足**：深度学习主要使用原始价格序列，CatBoost使用892个全量特征
...
> **全量特征（892个）显著优于500特征，年化收益率提升10.14%，索提诺比率提升83%**

**验证背景**：
- 目的：对比全量特征和500个精选特征的性能差异
...
1. **年化收益率提升**：40.42% vs 30.28%（+10.14%）⭐
2. **索提诺比率提升**：1.9023 vs 1.0400（+83%）⭐
3. **夏普比率提升**：-0.0235 vs -0.0501（+53%）⭐
4. **平均收益率提升**：3.21% vs 2.40%（+34%）⭐
5. **买入信号胜率提升**：49.60% vs 49.13%（+0.47%）
6. **Fold表现改善**：8个Fold表现更好，4个Fold表现持平
...
**短期（立即执行）**：
- ✅ 所有训练命令移除 `--use-feature-selection` 参数
- ✅ 使用全量特征（892个）进行模型训练
- ✅ 更新所有文档和脚本说明
...
1. **验证必须公平**：
   - 必须使用相同的置信度阈值进行对比
   - Walk-forward 验证是唯一可信的评估方法
   - 避免在同一数据集上反复优化

2. **训练时间增加**：
   - 全量特征训练时间稍长
   - 需要评估计算资源成本
   - 建议：在训练服务器上运行，避免影响日常使用
...
**相关文件**：
- 全量特征验证报告：`output/walk_forward_sector_bank_catboost_20d_20260327_142106.md`
- 500特征验证报告：`output/walk_forward_sector_bank_catboost_20d_20260327_145815.md`
- 综合对比分析：`output/feature_comparison_final_20260327.md`
- 验证日志：`output/walk_forward_full_features_20260327.log`、`output/walk_forward_500_features_20260327.log`

**最终推荐**：
- ✅ **立即使用全量特征（892个）**（综合性能提升显著，验证可信）
- ⚠️ 持续监控实盘交易结果
- ⚠️ 定期重新验证，确保优势持续存在
...
#### 支持的方法
- **使用全量特征（892个）**：CatBoost 的自动特征选择机制⭐ **当前推荐（2026-03-27验证）**
- **统计方法（statistical）**：F-test + 互信息混合方法（已废弃）
- **模型重要性法**：基于特征对模型预测的贡献度进行选择（已废弃）
- **累积重要性法（cumulative_importance）**：基于累积重要性阈值自动决定特征数量（实验性，已废弃）
- **随机抽样优化**：在特征选择时使用随机抽样提升速度（已废弃）
...
1. **全量特征在7/9个指标上优于500特征**
   - 年化收益率提升10.14%，这是显著的性能提升
   - 索提诺比率提升83%，说明风险调整后的收益大幅改善
   - 夏普比率提升53%，更接近0（无风险基准）

2. **为什么全量特征更好？**
   - **CatBoost 的自动特征选择机制**：CatBoost 内置 L2 正则化和自动特征重要性计算，能够自动降权不重要特征
   - **信息保留完整**：892个特征包含所有信息，避免特征选择导致的信息丢失
   - **特征组合保留**：520个交叉特征可能包含重要的非线性关系
   - **减少预选误差**：避免人为特征选择引入的偏差
...
4. **业界实践对比**
   - 之前认为"业界90%使用300-500特征"的结论不适用于 CatBoost
   - CatBoost 的 Ordered Boosting 算法和 L2 正则化使其能够有效处理大量特征
   - 与 Bloomberg、Two Sigma 等顶级量化机构相比，他们使用 LightGBM/XGBoost，而 CatBoost 有不同的优化策略
...
⚠️ **重要警告**：
- 之前的对比结果（2026-03-17显示500特征更好）是由于置信度阈值不一致导致的错误结论
- 必须使用相同的置信度阈值进行公平对比（本次验证均使用0.60）
- Walk-forward 验证是唯一可信的评估方法
...
### 特征工程
- **特征数量**：**892个全量特征** ⭐ **当前推荐（2026-03-27验证）**
- **预测周期**：1天、5天、20天
- **特征选择策略**：使用全量特征，依赖 CatBoost 的自动特征选择机制
- **业界最佳实践更新**：
  - **CatBoost 特性**：支持自动特征重要性计算和 L2 正则化，无需预先选择特征
  - **Ordered Boosting**：CatBoost 的独特算法，减少训练集-验证集信息泄露
  - **信息保留优先**：在 CatBoost 场景下，保留所有特征比预先筛选更优
- **全量特征 vs 500特征验证结果**（2026-03-27）：
  - 年化收益率：40.42% vs 30.28%（+10.14%）✅
  - 索提诺比率：1.9023 vs 1.0400（+83%）✅
  - 夏普比率：-0.0235 vs -0.0501（+53%）✅
  - 结论：全量特征在7/9个指标上优于500特征
- **特征类别**：
  - 滚动统计特征（偏度、峰度、多周期波动率）
  - 价格形态特征（日内振幅、影线比例、缺口）
  - 量价关系特征（背离、OBV、成交量波动率）
  ...
  - **事件驱动特征**（9个，2026-03-29新增）⭐：
    ...
    - **数据源**：AKShare（分红）、Yahoo Finance（财报）
    - **验证结果**（Walk-forward，阈值0.6）：
      - 索提诺比率大幅提升+51%（0.8125 → 1.2251）
      - 震荡市胜率显著提升：Fold 7 (+4.18%)、Fold 9 (+5.98%)
      - 准确率+0.74%，正确决策比例+0.63%
      - 年化收益率轻微下降：31.52% → 30.71%（-0.81%）
    - **结论**：事件驱动特征在风险控制和震荡市表现优异，值得保留

### 模型性能（来自 model_accuracy.json）

#### 单模型性能（含F1分数）
- **CatBoost 20天**：准确率 62.03%（±2.31%），F1分数 0.6432（±0.0311）⭐ **当前最佳（稳定可靠）**
- **GBDT 20天**：准确率 56.10%（±4.93%），F1分数 0.6597（±0.0538）
- **LightGBM 20天**：准确率 58.33%（±7.64%），F1分数 0.6943（±0.0613）
...
### 特征选择策略
- **统一策略**：使用全量特征（892个），依赖 CatBoost 的自动特征选择机制
- **验证结果**（2026-03-27 Walk-forward 验证）：全量特征年化收益率 40.42% vs 500特征 30.28%（+10.14%）
- **优势**：保留所有信息，避免特征选择导致的信息丢失
- **性能**：训练速度稍慢，但性能提升显著，值得投入
...
### 执行流程
1. **步骤1**：训练 CatBoost 20天模型（使用全量特征，892个）
2. **步骤2**：生成 CatBoost 单模型预测
3. **步骤3**：调用 `hsi_email.py --force --no-email` 生成大模型建议（不发送邮件）
4. **步骤4**：提取大模型建议中的买卖信息（包含推荐理由、操作建议、价格指引、风险提示）
5. **步骤5**：提取 CatBoost 预测结果中的上涨概率信息
6. **步骤6**：提交给大模型进行综合分析
7. **步骤7**：生成详细的综合买卖建议，包含：
   - 强烈买入信号（2-3只）
   - 买入信号（3-5只）
   - 持有/观望
   - 卖出信号（如有）
   - 风险控制建议
8. **步骤8**：发送邮件通知，包含完整信息参考章节
...
# 批量回测不同置信度阈值
python3 ml_services/batch_backtest.py --model-type catboost --horizon 20 --confidence-threshold 0.55

# 20天持有期回测（支持自定义日期范围）⭐ 推荐
python3 ml_services/backtest_20d_horizon.py --start-date 2025-01-01 --end-date 2025-12-31 --horizon 20 --confidence-threshold 0.6
...
4. **全量特征优于500特征**（2026-03-27验证）：年化收益率40.42% vs 30.28%（+10.14%），索提诺比率1.9023 vs 1.0400（+83%）⭐⭐⭐⭐⭐
5. **不推荐使用融合模型**：信号稀释问题导致表现远不如CatBoost单模型
6. **不推荐使用深度学习模型**：LSTM F1分数为0，Transformer F1分数0.1303，无法实际交易
7. **特征选择策略**：使用全量特征（892个），依赖 CatBoost 的自动特征选择机制
8. **市场环境自适应**：ADX+波动率双因子识别，Fold 7收益率从-1.16%提升至+6.24%
9. **数据泄漏修正**：系统性修正10+个特征，使用.shift(1)确保滞后数据
10. **回撤计算修正**：多周期策略使用非重叠样本，回撤从-65.45%修正至-13.12%
11. **夏普比率计算修正**：添加无风险利率（2%），修正年化标准差公式，使用买入信号标准差
...
- ✅ **使用全量特征（892个）**（2026-03-27验证：年化收益率+10.14%，索提诺比率+83%，优于500特征）
...
- ✅ 交易层：完整，模拟交易系统正常运行
- ✅ 服务层：完整，大模型服务集成
- ✅ **特征策略**：完整，使用全量特征（892个）+ 事件驱动特征（9个）
- ✅ **模型可重现性**：完整，使用固定随机种子（seed=42）
...
- ✅ **事件驱动特征**：完整，9个新特征（分红、财报日期、财报超预期），索提诺比率+51%，震荡市胜率显著提升
- ✅ **板块Walk-forward验证**：完整，业界标准方法，消费股排名第1（夏普比率0.7445）
- ✅ **市场环境自适应**：完整，基于ADX+波动率
- ✅ **数据泄漏修正**：完整，系统性修正10+个特征
- ✅ **批量回测**：完整，支持28只股票
- ✅ **自动化**：完整，12个GitHub Actions工作流
...
- `feature_selection_methods_comparison.md`：特征选择方法对比 ⚠️ 已废弃
- `feature_selection_summary.md`：特征选择总结 ⚠️ 已废弃
