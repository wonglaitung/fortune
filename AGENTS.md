...
**个股完整模型**（38 folds，59只股票，市场情绪过滤器启用，PIT 口径；**2026-09-26 复测**，20d=LightGBM / 1d·5d=CatBoost，按 D10 分周期选型）⭐：

| 指标 | 20d LightGBM | 5d CatBoost | 1d CatBoost |
|------|-----|----|----|
| 合并准确率 | **51.8%** [49.7, 53.9]（p=0.105 不显著） | **51.7%** [50.6, 52.7]（p=0.0018） | **51.0%** [50.5, 51.4]（p=0.0001） |
| 信号胜率 / 基准胜率 | 51.3% / 49.5% | 48.2% / 46.9% | 40.6% / 39.1% |
| **超额 lift** | **+1.9pp**（p=0.240 不显著） | +1.2pp（p=0.149 不显著） | +1.5pp（p=0.0003 显著） |
| 月度护栏 净IR [95%CI] | **1.41** [0.32, 2.62] → 🟢 | 0.74 [−0.35, 1.74] → 🟡 | −1.96 → 🔴 |
| PBO / DSR | 0.19 / 0.982 | 0.04 / 0.796 | 0.49 / 0.000 |
| 综合评分 / 平均夏普 | 77 良好 / 1.45 | 77 良好 / 1.27 | 72 良好 / 1.43 |
| 平均 IC（20d rank_ic 0.076） | 0.055 | — | — |

> 上轮（2026-09-25）20d 为 52.3% / lift +2.3pp（p=0.157）、5d 为 51.8% / lift +1.6pp → 本轮**小幅回落但仍同量级**；
> **1d 虽 lift 显著但净IR −1.96 → 按 D10/D2 停用**（微弱方向信息 ≠ 可交易组合）。
