...
#### 模型性能（2026-02-24 最新，来自 model_accuracy.json）

**单模型性能**：
- **CatBoost 20天**：准确率 61.22%（±2.56%）⭐ **当前最佳（稳定可靠）**
- **CatBoost 5天**：准确率 63.01%（±4.45%）⚠️ 谨慎使用（需要更多验证）
- **CatBoost 1天**：准确率 65.62%（±5.97%）❌ **不推荐使用**（存在严重过拟合风险）
- **GBDT 20天**：准确率 60.42%（±5.15%）
- **LightGBM 20天**：准确率 59.53%（±4.73%）
...
- 更快的训练速度，支持 GPU 加速
- 更好的泛化能力，减少过拟合
- **稳定性显著提升**（±2.56% vs LightGBM ±4.73%，提升 45.4%）
...
7. **## 六、股票技术指标详情**（推荐股票的技术指标表格）
8. **## 七、技术指标说明**（短期、中期技术指标说明）
9. **## 八、决策框架**（买入、持有、卖出策略说明）
10. **## 九、风险提示**（模型不确定性、市场风险、投资原则）
11. **## 十、数据来源**（11个数据源说明）
...
**ML 模型状态**（2026-02-24 最新，来自 model_accuracy.json）:
- ✅ **单模型性能**：
  - **CatBoost 20天**：准确率 61.22%（±2.56%）⭐ **当前最佳（稳定可靠）**
  - **CatBoost 5天**：准确率 63.01%（±4.45%）⚠️ 谨慎使用（需要更多验证）
  - **CatBoost 1天**：准确率 65.62%（±5.97%）❌ **不推荐使用**（存在严重过拟合风险）
...
- ✅ **批量回测功能（2026-02-22 新增）**：
  ...
  - 批量回测结果（置信度0.55）：
    - CatBoost 20天：平均总收益率（基于2年回测）238.76%，夏普比率1.51，胜率32.81%
    - 融合模型：平均总收益率（基于2年回测）115.13%，夏普比率1.00，胜率31.89%
    - GBDT 20天：平均总收益率（基于2年回测）-1.86%，夏普比率-0.06，胜率29.88%
    - LightGBM 20天：平均总收益率（基于2年回测）-8.22%，夏普比率-0.18，胜率29.57%
  - 批量回测结果（置信度0.60）：
    - CatBoost 20天：平均总收益率（基于2年回测）206.72%，夏普比率1.52，胜率31.84%
    - 融合模型：平均总收益率（基于2年回测）75.97%，夏普比率0.86，胜率30.97%
    - GBDT 20天：平均总收益率（基于2年回测）-13.02%，夏普比率-0.31，胜率25.11%
    - LightGBM 20天：平均总收益率（基于2年回测）-14.96%，夏普比率-0.24，胜率26.47%
  - **置信度阈值对比分析（0.50、0.55、0.60）**：
    - CatBoost最佳置信度0.50：年化收益率88.75%，胜率32.81%
    - 融合模型最佳置信度0.50：年化收益率46.29%，胜率31.89%
    - 提高置信度阈值导致年化收益率下降
    - 置信度阈值 ≠ 预测准确率
...
- ✅ 独立运行支持（使用默认准确率值）
- ✅ **邮件内容结构优化**（主要部分为综合买卖建议，信息参考部分包含11个子章节）
...
**批量回测结果**（置信度0.60，28只股票，2026-02-22）：
- **CatBoost 20天**：平均总收益率（基于2年回测）206.72%，夏普比率1.52，胜率31.84%，**优秀股票23只（收益率>50%）**
- **融合模型（加权平均）**：平均总收益率（基于2年回测）75.97%，夏普比率0.86，胜率30.97%，**优秀股票15只（收益率>50%）**
- **GBDT 20天**：平均总收益率（基于2年回测）-13.02%，夏普比率-0.31，胜率25.11%，**优秀股票1只（收益率>50%）**
- **LightGBM 20天**：平均总收益率（基于2年回测）-14.96%，夏普比率-0.24，胜率26.47%，**优秀股票0只（收益率>50%）**
...
| 模型 | 准确率 | 标准偏差 | 可信度 | 建议 |
|------|--------|----------|--------|------|
| CatBoost 20天 | 61.22%（±2.56%） | 高 | 推荐 |
| CatBoost 5天 | 63.01%（±4.45%） | 中 | 谨慎使用 |
| **CatBoost 1天** | **65.62%（±5.97%）** | **低** | **❌ 不推荐（过拟合风险）** |
| GBDT 20天 | 60.42%（±5.15%） | 中 | 可用 |
| LightGBM 20天 | 59.53%（±4.73%） | 中 | 可用 |
| LightGBM 1天 | 51.91%（±2.25%） | 高 | 可用 |
| GBDT 1天 | 51.59%（±1.61%） | 高 | 可用 |
| LightGBM 5天 | 56.47%（±2.27%） | 中 | 可用 |
| GBDT 5天 | 55.19%（±2.54%） | 中 | 可用 |

**CatBoost vs LightGBM 稳定性对比**：
- CatBoost 20天：±2.56% vs LightGBM 20天：±4.73%
- **稳定性提升 45.4%**
...
**2026-02-18 回测评估功能**：
- 验证模型盈利能力
- LightGBM：总收益率（基于2年回测）60.58%，夏普比率 1.35
- GBDT：总收益率（基于2年回测）135.46%，夏普比率 2.06
...
- commit 33b73d3: 更新 AGENTS.md 文件并清理旧的文档和数据文件
- commit 037874c: docs: move documentation files and add ml_trading_model evaluation report
- commit 8987ac5: feat: 升级 ml_services 模块日志系统
- commit 8c555df: feat: 更新美股市场数据获取模块的超时控制和特征选择优化
- commit 65233e2: docs: 更新所有MD文档，添加最新的融合模型性能对比和批量回测结果
- commit bbec739: docs: 修正收益率描述，明确标注周期或使用年化收益率
- commit 59ef6f2: docs: 修正回测周期描述，明确标注回测时长而非预测周期
- commit 5372b9a: docs: 更新AGENTS.md，为所有平均总收益率添加回测周期标注

---
最后更新：2026-02-24（CatBoost 算法集成、三模型融合、三分类预测、小市值板块支持、置信度和一致性计算优化、批量回测功能、置信度阈值对比分析、CatBoost vs GBDT 表现差异分析、优秀股票标记、最新模型准确率数据、CatBoost批量回测详细表现）
</content>