...
| **异常检测** | `python3 crypto_email.py --mode quick` (快速) / `--mode deep` (深度) ⭐ |
...
| `comprehensive-analysis.yml` | 综合分析邮件 | 周一到周五 UTC 8:00 |
| `hsi-prediction.yml` | 恒生指数涨跌预测 | 周一到周五 UTC 22:00 |
| `bull-bear-analysis.yml` | 牛熊市分析 | 每周一 UTC 17:00 |
| `sector-analysis.yml` | 板块表现分析 | 每月1号 UTC 19:00 |
| `ranking-analysis.yml` | 股票表现排名分析 | 每月1号 UTC 19:00 |
| `performance-monitor.yml` | 预测性能月度报告 | 每月1号 UTC 20:00 |
...
- [progress.txt](progress.txt) - 项目进度跟踪
