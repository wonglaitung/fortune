...
| **恒指 vs 个股** | 恒指准确率显著高于个股（81% vs 57%），个股预测接近随机，不可单独依赖（见 lessons.md） |
...
### 可用策略（恒指增强模型验证，2026-04-28，33特征）

| 策略 | 胜率 | 操作 |
|------|------|------|
| **假突破(101)做多** | **95.00%** | ⭐⭐⭐⭐⭐ 最优策略 |
| **反弹失败(010)做多** | **85.98%** | ⭐⭐⭐⭐⭐ 次优策略 |
| **下跌中继(001)做多** | **84.00%** | ⭐⭐⭐⭐ |
| 一致看跌(000)做空 | 79.57% | ⭐⭐⭐⭐ 样本最多 |
| 一致看涨(111)买入 | 80.62% | ⭐⭐⭐⭐ |
...
**优化效果**：夏普比率 0.9153（+12.6%），最大回撤 -0.21%（+60%）
...
- **股票关联分析**：[docs/STOCK_CORRELATION_ANALYSIS.md](docs/STOCK_CORRELATION_ANALYSIS.md) - 领头羊股票、跨板块相关性
- **经典交易理论**：[docs/CLASSIC_TRADING_THEORIES.md](docs/CLASSIC_TRADING_THEORIES.md)

---

**最后更新**：2026-04-29（参数优化：恒指20天准确率81.24%，假突破策略95.00%，反弹失败策略85.98%）
