...
- **CatBoost 20天**：平均总收益率206.72%，夏普比率1.52，胜率31.84%，**优秀股票28只（100%）** ⭐
- **融合模型（加权平均）**：平均总收益率140.69%，夏普比率1.01，胜率29.20%，**优秀股票28只（100%）** ⭐
- **GBDT 20天**：平均总收益率-1.63%，夏普比率0.10，胜率29.13%，**优秀股票5只（18%）**
- **LightGBM 20天**：平均总收益率-1.85%，夏普比率0.10，胜率28.63%，**优秀股票4只（14%）**

**批量回测结果**（置信度0.55，28只股票，2026-02-22）：
- **CatBoost 20天**：平均总收益率238.76%，夏普比率1.51，胜率32.81%，**优秀股票24只（86%）** ⭐
- **融合模型（加权平均）**：平均总收益率115.13%，夏普比率1.00，胜率31.89%，**优秀股票22只（79%）** ⭐
- **GBDT 20天**：平均总收益率-1.86%，夏普比率-0.06，胜率29.88%，**优秀股票4只（14%）**
- **LightGBM 20天**：平均总收益率-8.22%，夏普比率-0.18，胜率29.57%，**优秀股票2只（7%）**

**批量回测结果**（置信度0.60，28只股票，2026-02-22）：
- **CatBoost 20天**：平均总收益率206.72%，夏普比率1.52，胜率31.84%，**优秀股票23只（82%）**
- **融合模型（加权平均）**：平均总收益率75.97%，夏普比率0.86，胜率30.97%，**优秀股票15只（54%）**
- **GBDT 20天**：平均总收益率-13.02%，夏普比率-0.31，胜率25.11%，**优秀股票1只（4%）**
- **LightGBM 20天**：平均总收益率-14.96%，夏普比率-0.24%，胜率26.47%，**优秀股票0只（0%）**
...
| 模型 | 置信度0.50收益率 | 置信度0.55收益率 | 置信度0.60收益率 | 收益变化 | 置信度0.50胜率 | 置信度0.55胜率 | 置信度0.60胜率 | 胜率变化 |
|------|----------|----------|----------|---------|----------|----------|----------|---------|
| **CatBoost 20天** | **206.72%** | **238.76%** | 206.72% | 0.55 > 0.50/0.60 | 31.84% | **32.81%** | 31.84% | 0.55最高 |
| **融合模型** | **140.69%** | 115.13% | 75.97% | 0.50 > 0.55 > 0.60 | **29.20%** | 28.40% | 27.30% | 0.50最高 |
| GBDT 20天 | -1.63% | -1.86% | -13.02% | 0.50最佳 | 29.13% | 28.20% | 25.11% | 0.50最佳 |
| LightGBM 20天 | -1.85% | -8.22% | -14.96% | 0.50最佳 | 28.63% | 27.50% | 26.47% | 0.50最佳 |

**关键发现**：
1. **CatBoost最佳置信度为0.55**：收益率最高（238.76%），胜率最高（32.81%）
2. **融合模型最佳置信度为0.50**：收益率最高（140.69%），所有股票都盈利（100%成功率）
3. **置信度0.50是通用最佳选择**：CatBoost和融合模型都实现100%成功率
4. **置信度阈值 ≠ 预测准确率**：提高置信度不一定提高胜率和收益率
5. **CatBoost模型最稳定**：在所有置信度下都表现优异，是首选模型
