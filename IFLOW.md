...
### 核心原则
1. **优先检查是否已有实现** - 在开始编码前，先搜索项目中是否已有类似功能或可复用的代码
2. **公共代码提取优先** - 若无现有实现，先新增公共函数，再在当前上下文中调用
3. **避免内联重复逻辑** - 严禁复制粘贴相同或相似的代码
4. **需求分析优先** - 深入理解用户需求，不要急于编码
5. **整体设计思维** - 考虑改动对整个系统的影响
6. **可维护性优先** - 考虑长期维护和扩展性
7. **修改完即测试** - 每次修改后立即验证，避免累积错误
...
*   `hsi_email.py`: 恒生指数价格监控器，基于技术分析指标生成买卖信号，只在有交易信号时发送邮件，集成股息信息追踪功能和AI持仓分析功能。
