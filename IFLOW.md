...
*   `generate_sector_rotation_river_plot.py`: **板块轮动河流图生成工具**，可视化展示各板块排名流动规律
...
### 板块轮动河流图生成工具
1. **功能概述**：生成展示过去半年内各个板块排名变化的河流图，可视化板块轮动趋势
2. **技术特点**：
   - 基于matplotlib的河流图可视化
   - 自动获取板块历史表现数据
   - 支持180天（半年）数据展示
3. **使用方法**：
   ...
   ```
4. **数据源**：通过腾讯财经接口获取港股数据
5. **可视化效果**：清晰展示板块轮动规律，辅助投资决策

### 性能优化
1. **API超时设置**：避免程序卡顿，提升稳定性
2. **数据缓存优化**：减少重复API调用
3. **大模型参数优化**：修复max_tokens参数超出API限制问题

### 界面优化
1. **邮件排版优化**：将过长的等号分隔符改为简洁的Markdown格式
2. **板块名称本地化**：修复板块轮动河流图中板块名称显示问题
3. **自选股分析标题排版**：提升邮件显示效果
...
4. 识别板块龙头股票（**业界标准MVP模型**：动量+成交量+基本面综合评分）
5. 分析板块资金流向（基于成交量和涨跌幅）
6. 生成板块分析报告，包含板块详细排名（含龙头股TOP 3）
7. 使用腾讯财经接口获取板块内股票数据
...
11. **业界标准MVP模型**（2026-02-13新增）：
    - 采用业界标准龙头股识别模型，综合评估动量（Momentum）、成交量（Volume）、基本面（Popularity/Fundamental）
    - 支持多周期分析：1日、5日、20天
    - 支持投资风格配置：进取型（动量60%）、稳健型（平衡）、保守型（基本面50%）
...
- **业界标准龙头识别**（2026-02-13升级）：基于MVP模型（动量+成交量+基本面）识别板块龙头股票
- **资金流向分析**：基于成交量和涨跌幅分析板块资金流入/流出情况
- **多周期分析**：支持1日、5日、20天三个周期的龙头股识别
- **投资风格配置**：支持进取型、稳健型、保守型三种风格，动态调整权重
- **市值筛选**：支持设置最小市值阈值（默认100亿港币）
...
**自动化状态**:
- ✅ GitHub Actions 工作流：7个主要工作流正常运行
- ✅ 邮件通知系统：163邮箱服务稳定运行
...
**可视化功能**:
- ✅ 板块轮动河流图：新增可视化工具，展示板块轮动规律
- ✅ 邮件排版优化：提升邮件显示效果

**性能优化**:
- ✅ API超时设置：避免程序卡顿
- ✅ 数据缓存优化：减少重复API调用
- ✅ 大模型参数优化：修复max_tokens限制问题
...
8. **最新改进**（2026-02-13）：板块龙头股识别功能升级为业界标准MVP模型，支持多周期分析和投资风格配置
9. **最新改进**（2026-02-13）：新增板块轮动河流图生成工具，可视化展示各板块排名流动规律
10. **最新改进**（2026-02-13）：邮件界面优化，添加龙头股TOP 3列，删除重复的强势/弱势板块表格
11. **安全性改进**（2026-02-13）：更新.gitignore防止敏感文件被提交，清理git历史记录
12. **最新改进**：在模拟交易系统中增加了更详细的交易决策说明，包括买卖原因的邮件通知
13. **新增功能**：集成通用技术分析工具，提供全面的技术指标分析能力
14. **新增功能**：增加恒生指数大模型策略分析器，提供专业的恒生指数交易策略
...
105. **性能优化**：新增API超时设置，避免程序卡顿问题
106. **界面优化**：自选股分析标题排版优化，提升邮件显示效果
107. **大模型优化**：修复max_tokens参数超出API限制问题，提升稳定性
108. **板块轮动河流图**：新增可视化工具，展示板块轮动规律，辅助投资决策
