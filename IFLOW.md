...
*   `ml_trading_model.py`: **机器学习交易模型**，基于LightGBM和GBDT+LR的二分类模型，预测1天、5天、20天后的涨跌，整合技术指标、基本面、资金流向、美股市场等103个特征。
...
3. **模型性能**：
   - 平均验证准确率：约50% (±2.4%) - 接近随机猜测，说明预测次日涨跌非常困难
   - 使用时间序列交叉验证（5折）
   - 特征重要性分析：美股市场特征和恒生指数特征在Top 10中占据主要位置
4. **训练流程**：
   - 获取24只自选股的2年历史数据
   - 计算所有技术指标、基本面、美股市场特征
   - 创建标签（N天后涨跌）
