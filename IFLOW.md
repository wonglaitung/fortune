...
2. **特征工程**：整合80+个技术指标 + 美股市场特征 + 基本面特征 + 股票类型特征 + 情感指标 + 技术-基本面交互特征
...
61. **架构重组**：将 ML 模块整合到 ml_services 目录，提高代码组织性和可维护性
62. **Bug修复**：修复机器学习模型导入错误，将相对导入改为绝对导入
...
71. **新增功能**：在hk_smart_money_tracker.py中集成ML模型关键指标（VIX_Level、成交额变化率、换手率变化率），采用业界标准的0-5层分析框架，提升分析准确性
72. **新增功能**：在hsi_email.py中集成ML模型关键指标和0-5层分析框架，针对短期和中期投资者提供不同的分析重点调整
73. **功能优化**：在hsi_email.py的交易信号总结表中添加成交额变化1日和换手率变化5日两个关键流动性指标
74. **功能优化**：在hsi_email.py的指标说明中添加VIX恐慌指数、成交额变化率、换手率变化率的详细解释
75. **功能优化**：删除hsi_email.py中的快速决策参考表和决策检查清单，简化邮件内容，提高可读性
76. **新增功能**：添加股票类型特征到ML模型，支持13种股票类型分类（bank、tech、energy、utility、semiconductor、ai、new_energy、shipping、exchange、insurance、biotech、environmental、index）
77. **新增功能**：实现分类特征编码（LabelEncoder），解决LightGBM无法直接处理字符串数据的问题
78. **新增功能**：添加股票类型衍生特征（防御性评分、成长性评分、周期性评分、流动性评分、风险评分、银行/科技/周期股分析权重）
79. **性能优化**：实施全面的正则化增强，显著降低过拟合程度（次日-30.5%，一周-18.2%，一个月-13.5%）
...
84. **最新功能**：在hk_smart_money_tracker.py中集成新闻分析功能，从 `data/all_stock_news_records.csv` 读取最新新闻，添加到第5层辅助信息
...
88. **最新功能**：在hsi_email.py的第5层辅助信息中添加新闻分析说明，提供新闻分析原则和投资者类型权重
89. **最新功能**：实现情感分析模块（llm_services/sentiment_analyzer.py），提供四维情感评分（相关性、影响度、预期差、情感方向），支持批量分析和情感统计
90. **最新功能**：在ML模型中集成情感指标特征（6个基础特征+78个交互特征），从新闻数据中计算情感趋势特征
91. **最新功能**：在ML模型中添加18个技术指标与基本面的交互特征（RSI×PE、MACD×PB等），捕捉非线性关系
92. **最新功能**：修复基本面数据提取逻辑，使用正确的键名（fi_pe_ratio、fi_pb_ratio）
93. **最新功能**：添加情感特征默认值处理，当新闻数据不可用时返回默认值0，避免数据丢失
94. **最新功能**：修复数据泄漏问题，在dropna前删除全为NaN的列，避免删除所有训练数据
95. **性能提升**：包含情感指标和交互特征后，次日模型准确率从52.27%提升至52.98%（+0.71%）
