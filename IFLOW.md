...
13. **机器学习交易模型**（基于LightGBM和GBDT+LR的多周期涨跌预测模型，支持1天、5天、20天预测，集成股票类型特征、情感指标、技术-基本面交互特征、板块分析特征）
...
*   `.github/workflows/hsi-email-alert-open_message.yml`: 强制发送恒生指数邮件工作流（支持 --force 参数），执行时间：周一到周五下午6:00香港时间（UTC 8:00）。
...
9. **ML模型集成**：提供17个板块分析特征（板块涨跌幅、板块排名、板块上涨比例、板块规模、板块趋势、板块资金流向、板块龙头、板块相对表现），预期提升模型准确率0.3-1.0%
10. **实时计算**：板块分析特征在每次ML模型训练时实时计算，确保数据的时效性和准确性
...
8. 支持缓存清除功能，方便强制更新数据。
9. **已知限制**：2800.HK（盈富基金）作为ETF，无法通过AKShare获取基本面数据（PE、PB等），训练时会自动处理缺失值。
...
2. **特征工程**：整合技术指标 + 美股市场特征 + 基本面特征 + 股票类型特征 + 情感指标 + 技术-基本面交互特征 + **板块分析特征** + **交叉特征**
   ...
   - **板块分析特征（17个）**：板块涨跌幅（1日/5日/20日）、板块排名（1日/5日/20日）、板块上涨比例（1日/5日/20日）、板块总成交量、板块股票数量、板块趋势评分、板块资金流向评分、是否为板块龙头、板块最佳/最差股票涨跌幅、板块是否跑赢恒指
   - **交叉特征（2000+个）**：13个分类特征 × 157-139个数值特征 = 1807-2041个交叉特征（Stock_Type、Outperforms_HSI_Stock_Type、Strong_Volume_Up_Stock_Type等与所有数值特征的交互）
3. **分类特征编码**：
   - 使用LabelEncoder将字符串类型的分类特征（如Stock_Type）转换为整数编码
   - 编码器在训练时保存到模型文件，在预测时加载使用
   - 支持处理训练时未见过的类别（使用默认值0）
   - 确保训练和预测时的特征编码一致性
4. **模型性能**（2026-02-08最新训练结果）：
   - **次日模型（1天）**：平均验证准确率 **50.93%** (LightGBM)，**51.51%** (GBDT+LR)
   - **一周模型（5天）**：平均验证准确率 **53.70%** (LightGBM)，**54.05%** (GBDT+LR)
   - **一个月模型（20天）**：平均验证准确率 **57.09%** (LightGBM)，**58.00%** (GBDT+LR) - **最佳表现**
   - 使用时间序列交叉验证（5折）
   - 特征重要性分析：HSI_Return_10d/20d/60d、US_10Y_Yield、VIX_Level等宏观指标在Top 10中频繁出现
   - **性能变化说明**：相比2026-01-30训练结果，性能略有波动，符合预期范围（50-58%），说明模型性能稳定，没有数据泄漏
   - **业界对比**：当前模型性能（50-58%）符合业界预期，无数据泄漏迹象，一个月模型准确率58.00%接近业界优秀水平（60%），具有实际交易价值
...
9. **注意事项**：
   ...
   - 需要定期验证实际预测准确率（建议每周重训练）
   - 建议与手工信号结合使用，不单独依赖
   - 一个月模型准确率58.00%接近业界优秀水平（60%），具有实际交易价值
   - 股票类型特征基于业界分析框架，不同类型股票应采用不同的分析权重
   - **性能监控**：当前模型性能在50-58%范围内，符合业界预期，没有数据泄漏
10. **业界差距分析**：
    - 详见 `ml_model_analysis_report.md` 文档
    - 当前模型准确率：51.51%（次日）、54.05%（一周）、58.00%（一个月）
    - 业界领先水平：65-70%（日内交易）、62-68%（波段交易）
    - 主要差距：特征数量（2000+ vs 500-1000）、模型数量（2 vs 5-10）、风险管理（缺失 vs 完善）
    - 改进建议：优化特征选择减少冗余、实现风险管理模块（VaR、止损止盈、仓位管理）、扩展时间序列特征
11. **板块特征集成**：
    - 新增17个板块分析特征（板块涨跌幅、板块排名、板块上涨比例、板块规模、板块趋势、板块资金流向、板块龙头、板块相对表现）
    - 板块分析数据通过 hk_sector_analysis.py 模块实时计算
    - 预期准确率提升：0.3-1.0%（保守估计）或 0.6-1.3%（中性估计）
    - 板块特征已完全集成到ML模型训练和预测流程中
...
5. **数据源切换**：AKShare获取标普500和纳斯达克数据不稳定时，自动切换到yfinance获取
6. **使用场景**：
   - 作为港股预测的外部市场环境特征
   - 捕捉美股对港股的影响
   - 提升模型预测准确率
...
│       │   ├── 特征工程（技术指标、市场环境、资金流向、基本面、美股市场、股票类型特征、情感指标、技术-基本面交互特征、板块分析特征）
...
**核心模块状态**:
- ✅ 数据获取层：完整，支持多数据源（yfinance、腾讯财经、AKShare）
- ✅ 数据服务层：完整，模块化架构（data_services/）
- ✅ 分析层：完整，包含技术分析、基本面分析、ML模型
- ✅ 交易层：完整，模拟交易系统正常运行
- ✅ 服务层：完整，大模型服务集成（llm_services/）

**自动化状态**:
- ✅ GitHub Actions 工作流：6个主要工作流正常运行
- ✅ 邮件通知系统：163邮箱服务稳定运行
- ✅ 定时任务：支持本地cron和GitHub Actions双重调度

**ML模型状态**:
- ✅ 次日模型：准确率 50.93%-51.51%（符合业界预期）
- ✅ 一周模型：准确率 53.70%-54.05%（良好）
- ✅ 一个月模型：准确率 57.09%-58.00%（接近业界优秀水平60%）
- ✅ 数据泄漏检测：已完成修复，无数据泄漏迹象
- ✅ 特征工程：2000+特征（技术指标、基本面、美股市场、股票类型、情感指标、板块分析）
...
**待优化项**:
- ⚠️ 风险管理模块：可添加VaR、止损止盈、仓位管理
- ⚠️ 深度学习模型：可考虑LSTM、Transformer等模型
- ⚠️ Web界面：可考虑添加可视化界面
...
- **最新功能**：在ML模型中集成17个板块分析特征（板块涨跌幅、板块排名、板块上涨比例、板块规模、板块趋势、板块资金流向、板块龙头、板块相对表现），预期提升模型准确率0.3-1.0%
...
- **注意**：所有ML模型文件和特征重要性文件已从Git跟踪中移除，避免仓库膨胀。运行`./train_and_predict_all.sh`会自动生成这些文件。
...
103. **工作流优化**：移除已废弃的工作流文件（hsi-email-alert.yml.bak、ml-prediction-alert.yml.bak、ml-train-models.yml.bak），简化工作流管理
104. **架构优化**：完成模块化重组，数据服务（data_services）、大模型服务（llm_services）、机器学习服务（ml_services）模块化完成，提高代码可维护性
