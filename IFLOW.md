...
20. **板块特征影响分析**（板块特征对ML模型准确率的影响分析，评估17个板块分析特征的预测性能提升）
...
*   `板块特征影响分析.md`: **板块特征对ML模型准确率的影响分析报告**，评估添加17个板块分析特征（板块涨跌幅、板块排名、板块上涨比例、板块规模、板块趋势、板块资金流向、板块龙头、板块相对表现）对机器学习交易模型准确率的影响，包含实验设计、预期影响分析和后续验证计划。
...
11. **板块特征影响分析**：
    ...
    - 验证计划：训练新模型、对比特征重要性、实际回测验证、长期监控
...
103. **新增功能**：新增 板块特征影响分析.md 文档，评估17个板块分析特征对ML模型准确率的影响，包含实验设计、预期影响分析（提升0.3-1.0%）和后续验证计划
104. **工作流优化**：移除已废弃的工作流文件（hsi-email-alert.yml.bak、ml-prediction-alert.yml.bak、ml-train-models.yml.bak），简化工作流管理
