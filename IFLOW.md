...
10. **新增功能**：采用业界标准的6层分析框架（风险控制→基本面质量→核心技术指标分析→TAV评分→信号识别→综合评分+新闻分析），提供系统性分析。
...
71. **新增功能**：在hk_smart_money_tracker.py中集成ML模型关键指标（VIX_Level、成交额变化率、换手率变化率），采用业界标准的6层分析框架，提升分析准确性
72. **新增功能**：在hsi_email.py中集成ML模型关键指标和6层分析框架，针对短期和中期投资者提供不同的分析重点调整
