...
#### 模型性能（2026-02-21 最新，来自 model_accuracy.json）

**单模型性能**：
- **CatBoost 1天**：准确率 65.62%（±5.97%）⭐ **当前最佳（过拟合风险高）**
- **CatBoost 20天**：准确率 61.09%（±1.50%）⭐ **稳定可靠**
- **CatBoost 5天**：准确率 63.01%（±4.45%）⚠️ 需要更多验证
- **LightGBM 20天**：准确率 58.87%（±4.38%）
- **GBDT 20天**：准确率 58.84%（±4.34%）
...
#### 回测测试结果（2026-02-22）
- **LightGBM 20天模型**：总收益率 11.73%，夏普比率 0.36，最大回撤 -9.69%，评级 ⭐⭐⭐ 一般
...
0728.HK,中国电信,weighted,1,0.6234,高,100%,4.91,2026-02-21,1,0.6117,1,0.6073,1,0.6512
```
...
**ML 模型状态**（2026-02-21 最新，来自 model_accuracy.json）:
- ✅ **单模型性能**：
  - **CatBoost 20天**：准确率 61.09%（±1.50%）⭐ **当前最佳（稳定可靠）**
...
- ✅ **回测评估功能（2026-02-22）**：
  ...
  - 测试验证（置信度阈值 0.55）：
    - LightGBM 20天模型：⭐⭐⭐ 一般（总收益率 11.73%，夏普比率 0.36）
    - GBDT 20天模型：⭐⭐⭐⭐⭐ 优秀（总收益率 73.99%，夏普比率 1.48）
    - CatBoost 20天模型：⭐⭐⭐⭐ 良好（总收益率 184.00%，夏普比率 1.62）
...
### 2026-02-20 至 2026-02-21 CatBoost 算法集成、模型融合与三分类预测

#### 优化背景
- 目标：进一步提升模型准确率和稳定性，探索多模型融合方法，实现更实用的三分类预测
- 初始状态：LightGBM 60.16%（±4.92%），GBDT 59.97%（±4.76%）
- 优化目标：集成 CatBoost 算法，实现三模型融合，支持上涨/观望/下跌三分类预测
...
| 模型 | 准确率 | 标准偏差 | 可信度 | 建议 |
|------|--------|----------|--------|------|
| CatBoost 20天 | 61.09%（±1.50%） | 高 | 推荐 |
| CatBoost 5天 | 63.01%（±4.45%） | 中 | 谨慎使用 |
| **CatBoost 1天** | **65.62%（±5.97%）** | **低** | **❌ 不推荐（过拟合风险）** |
| LightGBM 20天 | 58.87%（±4.38%） | 中 | 可用 |
| GBDT 20天 | 58.84%（±4.34%） | 中 | 可用 |
...
- CatBoost 20天：±1.50% vs LightGBM 20天：±4.38%
- **稳定性提升 65.8%**
...
|---------|--------|----------|---------------|
| 加权平均 | ~61-62%（±1.5-2.0%） | +0.5-1.5% ⭐ |
| 简单平均 | ~61.5%（±1.8%） | +0.3-0.8% |
| 投票机制 | ~61.2%（±2.0%） | +0.0-0.5% |
...
---
最后更新：2026-02-21（CatBoost 算法集成、三模型融合、三分类预测、小市值板块支持、置信度和一致性计算优化、最新模型准确率数据）