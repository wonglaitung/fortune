...
- 完善技术分析工具功能
- 优化用户体验和交互界面
- GitHub Actions 工作流 `.github/workflows/smart-money-alert.yml` 现已整合运行 `batch_stock_news_fetcher.py` 和 `hsi_llm_strategy.py`，实现了更全面的市场分析和策略生成
- 本地 `send_alert.sh` 脚本也已更新，包含个股新闻获取和恒生指数策略分析功能
- 恒生指数价格监控器（@hsi_email.py）基于技术分析指标生成恒生指数交易信号，并只在有信号时发送邮件
- 所有邮件发送组件（@crypto_email.py, @gold_analyzer.py, @hsi_email.py）只在当天发现信号时才发送邮件
