...
实践人机混合智能的理念，开发具备变现能力的金融资产智能量化分析助手，实时监控加密货币、港股、黄金等金融市场。香港股票方面集成11个数据源、机器学习模型和大模型智能决策。CatBoost 20天模型准确率60.88%（修复数据泄漏后），支持实时技术指标、板块轮动分析、综合买卖建议和自动化调度，以智能副驾的方式为投资者提供全面的市场分析和交易策略验证。
...
### 📊 板块模型真实性能（Walk-forward验证）

| 板块 | 股票数 | 胜率 | 夏普比率 | 最大回撤 | 推荐度 |
|------|-------|------|---------|---------|--------|
| **银行股** | 6 | **50.44%** | **0.155** | **-13.12%** | ⭐⭐⭐⭐⭐ |
| **半导体股** | 3 | 49.87% | 0.126 | -21.36% | ⭐⭐⭐⭐⭐ |

**关键改进**（2026-03-19）：
- ✅ **回撤计算修正**：多周期策略使用非重叠样本，银行股回撤从-65.45%修正至-13.12%
- ✅ **市场环境自适应**：基于ADX+波动率双因子，Fold 7收益率从-1.16%提升至+6.24%
- ✅ **数据泄漏修正**：系统性修正10+个特征，使用.shift(1)确保滞后数据

### 🚀 关键特性

- **CatBoost 单模型策略**：经过严格验证，CatBoost 单模型表现远优于融合模型和深度学习模型
- **板块Walk-forward验证**：业界标准的板块模型验证，评估真实预测能力
- **市场环境自适应过滤**：基于ADX+波动率双因子识别市场状态（震荡市/正常市/趋势市），动态调整过滤条件
- **风险管理特征集**：ATR动态止损、连续市场状态记忆、盈亏比评估等18个新特征
...
- **板块表现分析自动化**：每月1号自动执行，按股票类型分析模型准确度，识别表现最佳/最差板块
- **股票表现TOP 10排名分析**：每月1号自动执行，按不同指标（平均收益率、胜率、准确率）排名，识别综合优秀股票
- **筹码分布分析**：基于成交量的简单分箱法，计算筹码集中度、拉升阻力，辅助判断股票上涨潜力
- **数据泄漏修正**：系统性修正10+个存在数据泄漏的特征，使用.shift(1)确保滞后数据

### ⚠️ 重要警告

1. **数据更新**：本README中的性能数据基于2026-03-19的最新验证结果
2. **ADX数据泄漏已修复**：2026-03-19发现ADX计算使用当天High/Low导致数据泄漏，已修复为使用滞后数据（.shift(1)），CatBoost 20天准确率从71.76%修正为60.88%
3. **多周期策略回撤计算**：horizon>1时必须使用非重叠样本计算回撤，避免重叠持有期导致的回撤虚高（银行股从-65.45%修正至-13.12%）
3. **CatBoost 1天模型存在严重过拟合风险**：准确率63.09%（±4.33%），标准偏差过高，**不推荐使用**
4. **融合模型表现不如CatBoost单模型**：所有融合方法年化收益率均低于5%，建议优先使用CatBoost单模型
6. **深度学习模型（LSTM、Transformer）表现远不如CatBoost**：F1分数极低，回测无交易，**不推荐用于实际交易**
7. **推荐使用 CatBoost 20天模型**：准确率60.88%（修复ADX数据泄漏后），标准差2.06%，F1分数0.6416
...
- **特征工程**：520+个特征（技术指标、基本面、美股市场、情感指标、板块分析、长期趋势、主题分布、主题情感交互、预期差距、市场环境自适应、风险管理）
...
- 更快的训练速度，支持GPU加速
- 更好的泛化能力，减少过拟合
- 稳定性显著提升（±16.61% vs LightGBM ±3.87%）
- 实际表现优异：准确率61.14%，F1分数0.6920

### 板块特定模型真实性能（Walk-forward验证）

> **Walk-forward验证是业界标准的模型验证方法，每个fold重新训练模型，评估真实预测能力**

**银行股板块（6只股票，12-fold验证，2026-03-19）**：
...
| **年化收益率** | **45.92%** | 复合年化收益 |
| **夏普比率** | **0.155** | 风险调整收益 |
| **最大回撤** | **-13.12%** | 修正后（原-65.45%）✅ |
| **稳定性评级** | **中（良好）** | 收益率标准差2.97% |
...
**关键发现**：
- ✅ 银行股表现最佳：胜率突破50%，回撤仅-13.12%（符合防御性资产特性）
- ✅ 半导体股夏普比率最高：0.126，风险调整收益最优
- ⚠️ 所有板块真实胜率29-50%，需配合市场环境识别和风险控制

**重要说明**：
- Walk-forward验证严格避免数据泄漏，结果可信
- 之前报告的83-90%买入胜率是基于数据泄漏的错误评估
- 简单评估方法已被证伪，不再使用
...
| 模型 | 准确率 | F1分数 | 标准差 | 训练时间 | 推荐指数 |
|------|--------|--------|--------|----------|----------|
| **CatBoost** | **61.14%** ⭐ | **0.6920** ⭐ | **16.61%** | 快（1-2分钟） | ⭐⭐⭐⭐⭐ |
| **LSTM** | 51.79% | **0.0000** ❌ | ±0.78% | 慢（1-2分钟） | ⭐ |
| **Transformer** | 51.15% | 0.1303 | ±4.00% | 慢（2-3分钟） | ⭐ |

**关键发现**：
- ✅ CatBoost 绝对优势：准确率领先，F1分数优秀
...
### 最新模型准确率（2026-03-19，添加520+特征后）

| 预测周期 | 模型 | 准确率 | F1分数 | 标准差 | 推荐指数 | 变化 |
|---------|------|--------|--------|--------|----------|------|
| **20天（推荐）** | **CatBoost** | **60.88%** ⭐ | **0.6416** ⭐ | **2.06%** | ⭐⭐⭐⭐⭐ | ⬇️ -0.26% |
| 20天 | GBDT | 58.01% | 0.7002 | 8.24% | ⭐⭐⭐ | ⬇️ -1.98% |
| 20天 | LightGBM | 57.40% | 0.7090 | 8.68% | ⭐⭐ | ⬇️ -1.58% |
| 5天 | CatBoost | 63.96% | 0.6785 | 4.22% | ⚠️ 需要更多验证 | - |
| 5天 | GBDT | 55.31% | 0.6943 | 2.45% | ⭐⭐ | - |
| 5天 | LightGBM | 55.12% | 0.6924 | 2.17% | ⭐⭐ | - |
| **1天（不推荐）** | **CatBoost** | **63.09%** ❌ | **0.6022** ❌ | **4.33%** | ⭐ 过拟合风险 | - |
| 1天 | GBDT | 53.56% | 0.5151 | 1.80% | ⭐ | - |
| 1天 | LightGBM | 50.91% | 0.4995 | 1.07% | ⭐ | - |

**关键发现**（添加市场环境自适应、风险管理特征、数据泄漏修正后）：
- ✅ **CatBoost 20天模型**：准确率60.88%，F1分数0.6416，标准差2.06%（已修复ADX数据泄漏）
- ⚠️ **GBDT 20天略有下降**：准确率从59.99%降至58.01%（-1.98%），新特征对GBDT帮助有限
- ⚠️ **LightGBM 20天略有下降**：准确率从58.98%降至57.40%（-1.58%），新特征对LightGBM帮助有限
- 💡 **原因分析**：新添加的分类特征（Market_Regime）和复杂交互特征更适合CatBoost的自动分类特征处理能力

**重要提示**：
- ⭐ **推荐使用 CatBoost 20天模型**：准确率60.88%，F1分数0.6416，标准差2.06%（已修复ADX数据泄漏）
- ❌ **不推荐使用 CatBoost 1天模型**：存在严重过拟合风险，准确率63.09%（±4.33%）过高

### 批量回测性能（28只股票）

| 模型类型 | 准确率 | F1分数 | 标准差 | 推荐指数 |
|---------|--------|--------|--------|----------|
| **CatBoost 20天** ⭐ | **60.88%** | **0.6416** | **2.06%** | ⭐⭐⭐⭐⭐ |
| LightGBM 20天 | 57.40% | 0.7090 | 8.68% | ⭐⭐ |
| GBDT 20天 | 58.01% | 0.7002 | 8.24% | ⭐⭐⭐ |

**CatBoost 批量回测详细表现**（添加520+特征后）：
- 模型类型：CatBoost 20天
- 准确率：60.88%（已修复ADX数据泄漏）
- F1分数：0.6416
- 标准差：2.06%（稳定性提升）
- 标准差：4.50%（稳定性显著改善）
- 训练时间：1-2分钟
- 时间戳：2026-03-19 14:46
- 特征数量：520+（含市场环境自适应、风险管理特征）
...
| **ML模型** | ✅ 顶尖 | CatBoost 20天准确率61.14%，F1分数0.6920 |
| **批量回测** | ✅ 完整 | 支持28只股票批量回测 |
...
1. **ADX数据泄漏已修复（2026-03-19）**：
   - 发现ADX计算使用当天High/Low导致数据泄漏
   - 已修复为使用滞后数据（.shift(1)）
   - CatBoost 20天准确率从71.76%修正为60.88%

2. **CatBoost 1天模型存在严重过拟合风险**：
   - 准确率63.09%（±4.33%），标准偏差过高
   - **不推荐使用** CatBoost 1天模型的预测结果
   - **推荐使用** CatBoost 20天模型作为主要预测来源

3. **融合模型表现不如CatBoost单模型**：
   - 所有融合方法年化收益率均低于5%
   - 融合模型仅用于对比研究，不建议用于实际交易

4. **深度学习模型（LSTM、Transformer）表现远不如CatBoost**：
...
6. **数据验证**：严格的时间序列交叉验证，无数据泄漏，日期索引保留，按时间顺序排列（2026-03-19已修复ADX计算泄漏）
...
- ✅ 核心功能完成度：95%+
- ✅ CatBoost 20天模型：准确率61.14%，F1分数0.6920，标准差16.61%
- ✅ 综合分析系统：每日自动执行
- ✅ 自动化调度：11个GitHub Actions工作流
- ⚠️ 风险管理：待实现
