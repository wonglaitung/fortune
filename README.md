...
2. **ADX计算优化**：ADX指标计算使用滞后数据（.shift(1)），确保特征工程符合时序预测规范
...
7. **推荐使用 CatBoost 20天模型**：准确率60.88%，标准差2.06%，F1分数0.6416
...
1. **ADX计算优化**：
   - ADX指标计算使用滞后数据（.shift(1)）
   - 确保特征工程符合时序预测规范
   - CatBoost 20天准确率60.88%，标准差2.06%
...
6. **数据验证**：严格的时间序列交叉验证，无数据泄漏，日期索引保留，按时间顺序排列
