...
- **机器学习（数据驱动的预测）**：
  - CatBoost 20天模型准确率61.44%，预测个股涨跌方向
...
**机器学习预测模型**：
- **CatBoost 20天模型**：准确率61.44%（±1.75%），F1分数0.6416
...
- **机器学习模型仅供参考**：CatBoost 20天模型准确率61.44%，仅供参考，不构成投资建议
...
- **双层检测**：Z-Score + Isolation Forest（多维特征检测，默认模式）
...
- **趋势延续性验证**：两年数据分析证明"升的继续升，跌的继续跌"假设错误，实际是均值回归
- **抄底策略信号**：价格异常+当日下跌，5天胜率72%，10天胜率73%
- **风险预警信号**：Isolation Forest high 异常，5天收益-3.04%，建议减仓
...
- **CatBoost 20天模型**：主要使用的预测模型（准确率61.44%，F1分数0.6416）
- **板块特定模型**：为16个板块训练独立模型（银行、半导体、科技等）
- **多周期预测**：预测1天、5天、20天后的涨跌
- **特征工程**：918个全量特征（技术指标、基本面、美股市场、情感指标、异常检测等）
  - 滚动统计特征（偏度、峰度、多周期波动率）
  - 价格形态特征（日内振幅、影线比例、缺口）
  ...
  - **异常检测特征（26个）**：价格异常、成交量异常、波动率异常等特征
...
- 平均收益率：2.98%（20天持有期）
- 年化收益率：35.82%
- 夏普比率：**0.1546** ⭐⭐⭐⭐（业界标准，基于Fold收益率计算）
- 最大回撤：-13.30%
- 稳定性评级：中（良好）

**异常检测与动态策略优化**（两年数据验证，2024-04-01 至 2026-04-01）：
- **趋势延续性验证**：938个异常样本证明"升的继续升，跌的继续跌"假设**错误**
  - 趋势延续率：49-51%（接近随机）
  - 相关系数：-0.10 至 -0.14（弱负相关，均值回归）
- **抄底策略信号**：价格异常 + 当日下跌
  - 5天收益率：+4.12%，胜率：**72%** ⭐
  - 10天收益率：+6.88%，胜率：**73%** ⭐
...
**核心结论**："升的继续升，跌的继续跌"假设**错误**，实际是均值回归信号

#### 最强抄底策略信号
| 条件 | 5天收益 | 5天胜率 | 10天收益 | 10天胜率 | 策略 |
|------|--------|--------|---------|---------|------|
| **价格异常+当日下跌** | **+4.12%** | **71.7%** | **+6.88%** | **72.8%** | ✅ **抄底** |
| 价格异常+当日上涨 | +1.96% | 53.7% | +2.38% | 54.5% | ⚠️ 观望 |
| 成交量异常+当日下跌 | +0.65% | 53.4% | +1.94% | 57.9% | ⚠️ 谨慎 |

**推荐策略**：当检测到价格异常且当日下跌时，考虑抄底（胜率71.7%）
...
| **stock-anomaly-detection.yml** | 港股异常检测 | 每天凌晨2点（香港时间） | 港股价格和成交量异常检测，避免交易时段误报 |
...
基于两年数据验证（2024-04-20 至 2026-04-08，17,383小时数据，1,009个异常），**股票异常策略完全不适用于加密货币市场**：

| 异常类型 | 股票市场（港股） | 加密货币市场 |
|---------|----------------|-------------|
| **IF high → 减仓** | ✅ 有效（-3.04%，43%） | ❌ 无效（-0.05%，47.9%） |
| **Z-Score抄底** | ✅ 强烈推荐（+7.02%，100%） | ❌ 高风险（-0.36%，41.5%） |
| **均值回归** | ✅ 存在 | ❌ 不存在 |

**核心结论**：
- ❌ **不要将股票异常策略应用于加密货币**
- ✅ **加密货币异常只作为监控信号，不进行方向性交易**
- ✅ **需要为加密货币开发独立的异常策略**

**根本原因**：
- 市场机制不同（交易时间、涨跌幅限制、市场有效性）
- 异常持续时间不同（股票日内修复，加密货币可能持续数天）
- 均值回归效应在加密货币市场不存在
...
> **⚠️ 注意**：本章节的数据可能与最新验证结果不一致。最新的板块模型验证结果请参考 [progress.txt](progress.txt) 和 [AGENTS.md](AGENTS.md)。银行股最新夏普比率为 0.1546（2026-04-08 验证）。
...
2. **异常策略注意事项**：
   - ⚠️ **"升的继续升，跌的继续跌"假设错误**：两年数据验证证明是均值回归
   - ✅ **价格异常+当日下跌**是最强抄底信号（胜率72%）
   - ✅ **Isolation Forest high 异常**是风险预警信号（建议减仓）
   - ❌ **股票异常策略不适用于加密货币**：两年数据验证，加密货币均值回归不存在
   - ✅ 成交量异常预测能力较弱，谨慎使用

3. **数据验证**：严格的时间序列交叉验证，无数据泄漏，日期索引保留，按时间顺序排列
4. **数据源限制**：部分数据源可能有访问频率限制
5. **缓存机制**：基本面数据缓存7天，可手动清除
6. **交易时间**：模拟交易系统遵循港股交易时间
7. **风险提示**：本系统仅供学习和研究使用，不构成投资建议
8. **API密钥**：请妥善保管API密钥，不要提交到版本控制
...
# 2. 港股异常检测（统一入口）
python3 detect_stock_anomalies.py --mode standalone --mode-type deep --date 2026-04-09

# 3. 综合分析（整合大模型建议和CatBoost预测）
...
# 5. 加密货币异常检测
python3 crypto_email.py --mode deep --date 2026-04-09
```
...
**最后更新**: 2026-04-09 - 模型准确率更新、加密货币异常策略验证、银行股夏普比率修正
