...
- 整合2000+个特征（技术指标、市场环境、资金流向、基本面、美股市场、股票类型特征、情感指标、技术-基本面交互特征、板块分析特征、交叉特征）
- 时间序列交叉验证（5折）
- **最新验证准确率（2026-02-08）：50.93%（次日LightGBM）/51.51%（次日GBDT+LR）、53.70%（一周LightGBM）/54.05%（一周GBDT+LR）、57.09%（一个月LightGBM）/58.00%（一个月GBDT+LR）**
- 特征重要性分析和可解释性分析
- 分类特征编码（LabelEncoder）
...
- 次日和一周模型准确率在50-54%范围内，符合常见弱信号模型预期
- 建议与手工信号结合使用，不单独依赖
...
**特征重要性 Top 10**（2026-02-08）：
1. HSI_Return_10d（恒生指数10日收益率）
2. US_10Y_Yield（美国10年期国债收益率）
3. HSI_Return_60d（恒生指数60日收益率）
4. US_10Y_Yield_Change（美国10年期国债收益率变化率）
5. VIX_Level（恐慌指数绝对值）
6. HSI_Return_20d（恒生指数20日收益率）
7. NASDAQ_Return_5d（纳斯达克5日收益率）
8. NASDAQ_Return_20d（纳斯达克20日收益率）
9. SP500_Return_20d（标普500 20日收益率）
...
- ✅ **次日和一周模型准确率在50-54%范围内，符合常见弱信号模型预期**
- **性能监控**：当前模型性能在50-58%范围内，符合业界预期，没有数据泄漏
- 相比2026-01-30训练结果，性能略有波动，符合预期范围，说明模型性能稳定
...
**A**: 机器学习模型整合了2000+个特征（实际使用1881-2133个）：
- **技术指标特征**（80+个）：移动平均线、RSI、MACD、布林带、ATR、成交量比率、价格位置、涨跌幅、成交额变化率、换手率变化率等
- **市场环境特征**（3个）：恒生指数收益率、相对表现
- **资金流向特征**（5个）：价格位置、成交量信号、动量信号
- **基本面特征**（2个）：PE、PB（只使用实际可用的数据）
...
- 最终性能：次日51.34%、一周54.46%、一个月57.98%（LightGBM）
...
- reg_alpha: 0.1 → 0.15（+50%）
- reg_lambda: 0.1 → 0.15（+50%）

**效果**：一个月模型LightGBM从57.58%提升至57.98%（+0.40%）

#### 效果验证

| 特征 | 重要性排名 | 重要性 | 说明 |
|------|-----------|--------|------|
| `Kurtosis_20d` | Top 10 | 3.67% | 进入一个月模型Top 10 |
| `Skewness_20d` | Top 10 | 2.31% | 进入一个月模型Top 10 |
| `Volatility_120d` | Top 20 | 1.29% | 进入一个月模型Top 20 |
| `ATR_Ratio_120d` | Top 20 | 1.43% | 进入一个月模型Top 20 |
...
| 维度 | 当前实现 | 顶级机构 | 差距 |
|------|---------|---------|------|
| **特征数量** | 2000+ | 5000-10000 | ⚠️ 中等 |
| **模型数量** | 2（LightGBM、GBDT+LR） | 5-10 | ⚠️ 中等 |
| **模型准确率** | 58%（一个月） | 60-65%（业界优秀） | ✅ 接近 |
