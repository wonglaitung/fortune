...
实践人机混合智能的理念，开发具备变现能力的金融资产智能量化分析助手，实时监控加密货币、港股、黄金等金融市场。香港股票方面集成11个数据源、机器学习模型和大模型智能决策。CatBoost 20天模型准确率71.76%（添加520+特征后提升10.62%），支持实时技术指标、板块轮动分析、综合买卖建议和自动化调度，以智能副驾的方式为投资者提供全面的市场分析和交易策略验证。
...
2. **多周期策略回撤计算**：horizon>1时必须使用非重叠样本计算回撤，避免重叠持有期导致的回撤虚高（银行股从-65.45%修正至-13.12%）
3. **CatBoost 1天模型存在严重过拟合风险**：准确率63.09%（±4.33%），标准偏差过高，**不推荐使用**
4. **融合模型表现不如CatBoost单模型**：所有融合方法年化收益率均低于5%，建议优先使用CatBoost单模型
5. **深度学习模型（LSTM、Transformer）表现远不如CatBoost**：F1分数极低，回测无交易，**不推荐用于实际交易**
6. **推荐使用 CatBoost 20天模型**：准确率71.76%（+10.62%），标准差4.50%，F1分数0.7892（+0.0972）
...
|---------|------|--------|--------|--------|----------|------|
| **20天（推荐）** | **CatBoost** | **71.76%** ⭐ | **0.7892** ⭐ | **4.50%** | ⭐⭐⭐⭐⭐ | ⬆️ +10.62% |
...
- ✅ **CatBoost 20天显著提升**：准确率从61.14%提升至71.76%（+10.62%），F1分数从0.6920提升至0.7892（+0.0972）
...
**重要提示**：
- ⭐ **推荐使用 CatBoost 20天模型**：准确率71.76%（+10.62%），F1分数0.7892（+0.0972），标准差4.50%
...
|---------|--------|--------|--------|----------|
| **CatBoost 20天** ⭐ | **71.76%** | **0.7892** | **4.50%** | ⭐⭐⭐⭐⭐ |
| LightGBM 20天 | 57.40% | 0.7090 | 8.68% | ⭐⭐ |
| GBDT 20天 | 58.01% | 0.7002 | 8.24% | ⭐⭐⭐ |

**CatBoost 批量回测详细表现**（添加520+特征后）：
- 模型类型：CatBoost 20天
- 准确率：71.76%（原61.14%，提升+10.62%）
- F1分数：0.7892（原0.6920，提升+0.0972）
- 标准差：4.50%（稳定性显著改善）
- 训练时间：1-2分钟
