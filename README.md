...
- **批量回测功能（2026-02-22）**：
  ...
  - **置信度阈值对比分析（0.50、0.55、0.60）**：
    - CatBoost最佳置信度0.55：收益率238.76%，胜率32.81%
    - 融合模型最佳置信度0.50：收益率140.69%，胜率29.20%
    - 提高置信度阈值导致收益率和胜率下降
    - 置信度阈值 ≠ 预测准确率
...
|------|-----------|-----------|-----------|---------|
| **CatBoost 20天收益率** | 206.72% | **238.76%** | 206.72% | 0.55 |
| **CatBoost 20天胜率** | 31.84% | **32.81%** | 31.84% | 0.55 |
| **融合模型收益率** | **140.69%** | 115.13% | 75.97% | 0.50 |
| **融合模型胜率** | **29.20%** | 28.40% | 27.30% | 0.50 |
| GBDT 20天收益率 | -1.63% | -1.86% | -13.02% | 0.50 |
| LightGBM 20天收益率 | -1.85% | -8.22% | -14.96% | 0.50 |

**关键发现**：
1. **CatBoost最佳置信度0.55**：收益率238.76%，胜率32.81%
2. **融合模型最佳置信度0.50**：收益率140.69%，胜率29.20%
