...
**人机协同优势**：全面性（**定性+定量**）、可靠性（**双重验证**）、灵活性（适应**不同投资风格**）、可解释性
...
**双模型架构**：评分模型（规则模型）+ CatBoost模型（机器学习）

**多周期预测**：同时预测1天、5天、20天三个周期

**三周期准确率（Walk-forward验证，938样本）**：
...
**八大交易模式**（1天-5天-20天预测组合）：

| 模式 | 描述 | 占比 | 20天准确率 | 策略 |
|------|------|------|-----------|------|
| **110** | 震荡回调 | 13.9% | **90.00%** | ⭐ 最优做空 |
| 000 | 一致看跌 | 23.6% | 79.19% | 强烈卖出 |
| 111 | 一致看涨 | 13.6% | 70.31% | 强烈买入 |
| 100 | 冲高回落 | 13.5% | 87.4% | 短线看空 |
| 011 | 探底回升 | 10.9% | 81.4% | 抄底机会 |
...
**Walk-forward 验证结果**（40 folds，2022-09至2025-12）：
...
**关键发现**：
- 高交易量Fold（≥10笔）准确率61.39%，低交易量Fold仅42.04%
- 训练样本≥200时表现稳定
- 2023年表现最佳，需进一步研究市场环境适应性
...
| 模型 | 准确率 | F1分数 | 推荐指数 |
|------|--------|--------|----------|
| **CatBoost** | **60.88%** | **0.6416** | 强烈推荐 |
| LSTM | 51.79% | 0.0000 | 不推荐 |
| Transformer | 51.15% | 0.1303 | 不推荐 |
...
│   ├── hsi_ml_model.py                 # 恒指CatBoost模型
│   ├── hsi_walk_forward.py             # 恒指Walk-forward验证
...
├── 输出文件 (output/)
├── 数据文件 (data/)
│   └── hsi_prediction_history.json     # 恒指预测历史
└── 配置文件
    ├── config.py                       # 全局配置
...
# 恒生指数预测验证
python3 hsi_prediction.py --verify
...
# 恒指模型滚动训练
python3 ml_services/hsi_ml_model.py --mode rolling --window 18

# 恒指Walk-forward验证
python3 ml_services/hsi_walk_forward.py --horizon 20
```
