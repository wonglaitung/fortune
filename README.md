...
|------|-----------|-------|---------|------|
| 1天 | **54.1%** | [50.5%, 57.7%] | ✅ better (p=0.03) | 日内短线参考 |
| 5天 | 55.9% | [47.8%, 63.8%] | ❌ 不显著 | 周度持仓决策 |
| 20天 | 57.5% | [41.2%, 72.4%] | ❌ 不显著 | 月度投资方向 |
...
| 维度 | 🇭🇰 港股个股模型 | 🇨🇳 A股个股模型 |
|------|-----------------|----------------|
| **算法** | 1d/5d CatBoost、20d LightGBM（A/B 胜出） | CatBoost 梯度提升 |
| **特征总数** | 1023个 | 1077个 |
...
|------|------|----|------|
| 合并准确率 | **52.3%** [50.2, 54.4] | **51.8%** | 20d p=0.036 better；5d 接近随机 |
| 信号胜率 | 51.8% | 48.6% | 含行情 |
| 基准胜率 | 49.5% | 46.9% | 无条件买入 |
| **超额 lift** | **+2.3pp**（p=0.16 不显著） | +1.6pp | 微弱 |
| 平均 IC | 0.057 | 0.067 | 偏低 |

> 样本 43,610 条；逐年 lift +0.3~+1.6pp（2024/2026 弱、2023/2025 略好），无年度显著性。
...
**阈值分层**（2026-09-25 起 bear/weak 改**分位动态门槛**，normal 保持硬约束）：
...
> 自动入库维护）；绝对阈值 0.70/0.65 因 Isotonic 阶梯**空转**（0.65 与 0.60 通过率
> 完全相同）而弃用，回退链 CSV→快照→绝对值，见 [docs/DECISIONS.md](docs/DECISIONS.md) D8。
