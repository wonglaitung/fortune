...
| **验证方法** | Walk-forward, 38 folds（PIT 修正后） | Walk-forward, 12 folds |
| **20天准确率** | ~51.6%（PIT 修正后，接近随机） | 55-60%（待 PIT 复核） |
| **特征缓存加速** | 170x | 170x |
| **随机种子** | 42（固定） | 42（固定） |
...
**Walk-forward 验证结果**（2026-09-22 PIT 修正后，59只股票，38 folds，市场情绪过滤器启用）：

| 指标 | 20d | 5d | 评估 |
|------|-----|----|------|
| 合并准确率 | **51.6%** | **51.8%** | 接近随机（20d p=0.14） |
| 信号胜率 | 51.2% | 48.6% | — |
| 基准胜率 | 49.5% | 46.9% | 无条件买入 |
| **超额 lift** | +1.8pp | +1.6pp | 微弱 |
| 平均 IC | 0.052 | 0.067 | 偏低 |

> ⚠️ 早期"平均准确率 55–58%、综合评分 90/100"主要由**静态快照穿越**（网络/情感/主题
> 特征用最新值广播历史行）与全局特征选择穿越贡献，PIT 时点还原后已作废。
