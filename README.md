...
| 周期 | 准确率 | 特点 | 用途 |
|------|--------|------|------|
| 1天 | 51.05% | 噪音大，仅供参考 | 日内短线参考 |
| 5天 | 62.32% | 趋势确认，辅助判断 | 周度持仓决策 |
| **20天** | **80.66%** | **最可靠，主要决策依据** | 月度投资方向 |

**八大交易模式**（恒指增强模型，2026-05-14 验证）：

基于三周期预测结果（1/0表示涨/跌）组合形成8种交易信号，例如"110"表示1天涨、5天涨、20天跌。

| 模式 | 描述 | 20天准确率 | 策略 |
|------|------|-----------|------|
| **101** | 假突破（1天涨5天跌20天涨） | **88.61%** | ⭐ 最优做多，短期回调后中线上涨 |
| **001** | 下跌中继 | **83.02%** | 次优做多信号 |
| 110 | 震荡回调 | 82.93% | 中等 |
| 000 | 一致看跌 | 81.40% | 强烈卖出，三周期共振下跌 |
| 010 | 反弹失败 | 80.95% | 做空信号 |
| 111 | 一致看涨 | 75.66% | 强烈买入，三周期共振上涨 |

**四条交易法则**（恒指增强模型）：

| 法则 | 条件 | 准确率 | 应用场景 |
|------|------|--------|----------|
| **假突破做多** | 1天涨5天跌20天涨(101) | **88.61%** | 短期回调后中线做多 |
| 下跌中继做多 | 1天跌5天跌20天涨(001) | **83.02%** | 下跌后买入 |
| 一致看跌做空 | 三周期全看跌(000) | 81.40% | 趋势确认后减仓或做空 |
| 一致看涨买入 | 三周期全看涨(111) | 75.66% | 趋势确认后顺势加仓 |
...
|---------|------|--------|------|---------|--------|
| **Walk-forward**（12 folds，57只股票） | **20天** | **57.50%** | **0.2367** | **0.2542** | ⭐⭐⭐⭐ 推荐 |
...
**Walk-forward 验证结果（57只股票，12 folds，Top 500特征，2026-05-14）**：
...
| 平均最大回撤 | **-0.88%** | <-20% | ✅ 极佳 |
| 平均准确率 | 57.50% | >50% | ✅ |
| 平均 IC | **0.2367** | >0.05 | ✅ 优秀 |
| 平均 Rank IC | **0.2542** | >0.05 | ✅ 优秀 |
| 总体盈亏比 | **1.53** | >1.0 | ✅ 达标 |
...
**特征重要性（个股20天模型，Top 10，2026-05-14）**：

| 排名 | 特征 | 重要性 | 类别 |
|------|------|--------|------|
| 1 | net_comm_rank_HSI_Return_60d | 3.67 | **网络交叉** |
| 2 | Volatility_30pct | 1.49 | 波动类 |
| 3 | Volatility_70pct | 1.33 | 波动类 |
| 4 | BB_Width_MA60 | 1.33 | 技术指标 |
| 5 | net_comm_rank_US_10Y_Yield | 1.31 | **网络交叉** |
| 6 | Volume_Volatility | 1.26 | 成交量 |
| 7 | net_constraint_risk_VIX_Level | 1.25 | **网络交叉** |
| 8 | MA250_Slope | 1.14 | 趋势类 |
| 9 | sector_avg_change_1d | 1.12 | 板块类 |
| 10 | net_comm_rank_HSI_Regime_Duration | 1.09 | **网络交叉** |

> **关键发现**：网络交叉特征（`net_comm_rank_*`, `net_constraint_*`）占据 Top 10 中的 5 席，证明市场级特征与网络社区交叉后具有显著预测价值。
