...
实践人机混合智能的理念，开发具备变现能力的金融资产智能量化分析助手，实时监控加密货币、港股、黄金等金融市场。香港股票方面集成11个数据源、机器学习模型和大模型智能决策。模型回测准确率83.60%、胜率81.18%、平均收益率8.35%、夏普比率2.32，支持实时技术指标、板块轮动分析、综合买卖建议和自动化调度，以智能副驾的方式为投资者提供全面的市场分析和交易策略验证。
...
|------|---------------|------|
| **准确率** | 83.60% | 业界顶尖水平 |
| **胜率** | 81.18%（含成本） / 82.48%（无成本） | 综合评估优异 |
| **平均收益率** | 8.35%（含成本） / 8.67%（无成本） | 实际回测表现卓越 |
| **夏普比率** | 2.32（含成本） / 2.40（无成本） | 风险调整后收益优秀 |
| **优秀股票占比** | 100% (28/28) | 模型稳定性强 |
...
1. **数据更新**：本README中的性能数据基于2026-03-14的最新回测结果
2. **CatBoost 1天模型存在严重过拟合风险**：准确率63.09%（±4.33%），标准偏差过高，**不推荐使用**
2. **融合模型表现不如CatBoost单模型**：所有融合方法年化收益率均低于5%，建议优先使用CatBoost单模型
3. **深度学习模型（LSTM、Transformer）表现远不如CatBoost**：F1分数极低，回测无交易，**不推荐用于实际交易**
4. **推荐使用 CatBoost 20天模型**：准确率83.60%，胜率81.18%，平均收益率8.35%，夏普比率2.32
...
- 实际回测表现优异：准确率83.60%，胜率81.18%，平均收益率8.35%，夏普比率2.32
...
|------|--------|--------|--------|----------|-----------|----------|
| **CatBoost** | **83.60%** ⭐ | **待更新** ⭐ | **待更新** | 快（1-2分钟） | **8.35%** | ⭐⭐⭐⭐⭐ |
...
| 融合方法 | 平均收益率 | 夏普比率 | 买入信号胜率 | 说明 |
|---------|-----------|---------|------|------|
| CatBoost 单模型 | 8.35% | 2.32 | 81.18% | 主要使用 ⭐ |
| 加权平均 | 待更新 | 待更新 | 待更新 | 基于模型准确率加权 |
| 简单平均 | 待更新 | 待更新 | 待更新 | 三个模型概率平均 |
| 投票机制 | 待更新 | 待更新 | 待更新 | 多数投票决定方向 |
| 动态市场 | 待更新 | 待更新 | 待更新 | 根据市场状态动态选择 |
| 高级动态 | 待更新 | 待更新 | 待更新 | CatBoost主导（90%权重） |
...
- 买入信号数：4064
- 整体准确率：83.60%
- 胜率：81.18%（含成本）/ 82.48%（无成本）
- 平均收益率：8.35%（含成本）/ 8.67%（无成本）
- 夏普比率：2.32（含成本）/ 2.40（无成本）
...
| 预测周期 | 模型 | 准确率 | 胜率 | 平均收益率 | 夏普比率 | 推荐指数 |
|---------|------|--------|------|-----------|---------|----------|
| **20天（推荐）** | **CatBoost** | **83.60%** ⭐ | **81.18%** ⭐ | **8.35%** ⭐ | **2.32** ⭐ | ⭐⭐⭐⭐⭐ |
| 20天 | GBDT | 待更新 | 待更新 | 待更新 | 待更新 | ⭐⭐⭐ |
| 20天 | LightGBM | 待更新 | 待更新 | 待更新 | 待更新 | ⭐⭐ |
| 5天 | CatBoost | 待更新 | 待更新 | 待更新 | 待更新 | ⚠️ 需要更多验证 |
| 5天 | GBDT | 待更新 | 待更新 | 待更新 | 待更新 | ⭐⭐ |
| 5天 | LightGBM | 待更新 | 待更新 | 待更新 | 待更新 | ⭐⭐ |
| **1天（不推荐）** | **CatBoost** | **63.09%** ❌ | 待更新 | 待更新 | 待更新 | ⭐ 过拟合风险 |
| 1天 | GBDT | 待更新 | 待更新 | 待更新 | 待更新 | ⭐ |
| 1天 | LightGBM | 待更新 | 待更新 | 待更新 | 待更新 | ⭐ |

**重要提示**：
- ⭐ **推荐使用 CatBoost 20天模型**：准确率83.60%，胜率81.18%，平均收益率8.35%，夏普比率2.32
- ❌ **不推荐使用 CatBoost 1天模型**：存在严重过拟合风险，准确率63.09%（±4.33%）过高

### 批量回测性能（28只股票）

| 模型类型 | 准确率 | 胜率 | 平均收益率 | 夏普比率 | 优秀股票占比 |
|---------|--------|------|-----------|---------|-------------|
| **CatBoost 20天** ⭐ | **83.60%** | **81.18%** | **8.35%** | **2.32** | **100% (28/28)** |
| LightGBM 20天 | 待更新 | 待更新 | 待更新 | 待更新 | 待更新 |
| GBDT 20天 | 待更新 | 待更新 | 待更新 | 待更新 | 待更新 |
| 融合模型（加权平均） | 待更新 | 待更新 | 待更新 | 待更新 | 待更新 |
...
| **ML模型** | ✅ 顶尖 | CatBoost 20天准确率83.60%，胜率81.18%，平均收益率8.35% |
| **批量回测** | ✅ 完整 | 支持28只股票批量回测 |
...
- ✅ 核心功能完成度：95%+
- ✅ CatBoost 20天模型：准确率83.60%，胜率81.18%，平均收益率8.35%，夏普比率2.32
- ✅ 综合分析系统：每日自动执行
