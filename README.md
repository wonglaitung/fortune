...
1. 控制台输出：历史信号分析结果和统计信息
2. Excel报告：hk_smart_money_historical_report.xlsx（包含所有信号、建仓信号和出货信号三个工作表）
3. 与实时分析保持一致的信号检测逻辑和阈值设置
...
3. **港股主力资金追踪**：默认每天UTC时间2:00执行实时分析
