...
`set_key.sh` 脚本用于配置系统运行所需的环境变量，包括邮件服务和AI大模型API。

**环境变量说明**：

| 变量名 | 说明 | 示例值 |
|--------|------|--------|
| `YAHOO_SMTP` | SMTP服务器地址 | `smtp.163.com` / `smtp.gmail.com` |
...
2. **填写配置信息**：
   ...
   export YAHOO_EMAIL=YOUR_EMAIL@163.com              # 发件人邮箱（请替换）
   export YAHOO_APP_PASSWORD=YOUR_PASSWORD_HERE       # 邮箱授权码（请替换）
   export RECIPIENT_EMAIL=recipient1@gmail.com,recipient2@icbcasia.com  # 收件人（请替换）

   # AI大模型配置
   export QWEN_API_KEY=YOUR_QWEN_API_KEY_HERE         # 通义千问API密钥（请替换）
   echo $QWEN_API_KEY  # 验证配置
   ```
...
1. ⚠️ **安全性**：不要将 `set_key.sh` 文件提交到版本控制系统
2. ⚠️ **授权码**：使用邮箱授权码而非登录密码
3. ⚠️ **API密钥**：妥善保管通义千问API密钥，避免泄露
4. ⚠️ **SMTP配置**：确保SMTP端口（通常为465或587）未被防火墙拦截

**GitHub Actions配置**：

如果使用GitHub Actions自动化，需要在仓库设置中配置Secrets：
...
如果不想修改 `set_key.sh`，也可以直接在命令行设置：

```bash
export YAHOO_SMTP=smtp.163.com
export YAHOO_EMAIL=your-email@163.com
export YAHOO_APP_PASSWORD=your_password
export RECIPIENT_EMAIL=user1@gmail.com
