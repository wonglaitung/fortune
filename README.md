...
实践人机混合智能的理念，开发具备变现能力的金融资产智能量化分析助手，实时监控加密货币、港股、黄金等金融市场。香港股票方面集成11个数据源、机器学习模型和大模型智能决策。CatBoost 20天模型准确率60.88%，支持实时技术指标、板块轮动分析、综合买卖建议和自动化调度，以智能副驾的方式为投资者提供全面的市场分析和交易策略验证。
...
- **CatBoost 20天模型**：准确率60.88%，标准差2.06%，F1分数0.6416，推荐使用
- **CatBoost 1天模型**：准确率63.09%（±4.33%），标准偏差过高，存在严重过拟合风险，不推荐使用
- **融合模型**：所有融合方法年化收益率均低于5%，表现不如CatBoost单模型（年化收益率79.54%），仅用于对比研究
- **深度学习模型**：LSTM（F1分数0.0000）、Transformer（F1分数0.1303）表现远不如CatBoost，不推荐用于实际交易
- **多周期策略回撤计算**：horizon>1时必须使用非重叠样本计算回撤，避免重叠持有期导致的回撤虚高（银行股从-65.45%修正至-13.12%）
...
- 更快的训练速度，支持GPU加速
- 更好的泛化能力，减少过拟合
- 稳定性显著提升（±2.06% vs LightGBM ±3.87%）
- 实际表现优异：准确率60.88%，F1分数0.6416
...
| 板块 | 股票数 | 胜率 | 20天收益率 | 年化收益率 | 夏普 | 回撤 | 推荐度 |
|------|-------|------|-----------|-----------|------|------|--------|
| **银行股** | 6 | **50.44%** | **3.64%** | **45.92%** | **0.155** | **-13.12%** | ⭐⭐⭐⭐⭐ |
| **半导体股** | 3 | 49.87% | 1.06% | 13.36% | 0.126 | -21.36% | ⭐⭐⭐⭐⭐ |
| AI股 | 4 | 48.33% | 0.09% | 1.13% | 0.059 | -54.88% | ⭐⭐⭐ |
| 科技股 | 8 | 47.07% | 0.03% | 0.38% | -0.090 | -68.18% | ⭐⭐ |
| 交易所 | 1 | 20.00% | -0.27% | -3.40% | -5.909 | -0.30% | ⭐ |
...
| 概率区间 | 交易次数 | 20天平均涨幅 | 年化收益率 | 胜率 | 标准差 |
|---------|---------|-------------|-----------|------|--------|
| <0.50 | 2,358 | -6.69% | -84.29% | 12.6% | 9.28% |
| 0.50-0.55 | 485 | -0.24% | -3.02% | 40.6% | 8.66% |
| 0.55-0.60 | 537 | 3.47% | 43.72% | 60.0% | 10.34% |
| 0.60-0.65 | 613 | 6.14% | 77.36% | 79.8% | 10.14% |
| 0.65-0.70 | 644 | 8.15% | 102.69% | 89.1% | 9.58% |
| 0.70-0.75 | 553 | 9.25% | 116.55% | 93.9% | 9.74% |
| 0.75-0.80 | 449 | 10.43% | 131.42% | 98.0% | 9.98% |
| 0.80-0.85 | 329 | 10.77% | 135.70% | 96.7% | 8.20% |
| 0.85-0.90 | 269 | 11.66% | 146.92% | 99.3% | 8.82% |
| >0.90 | 91 | 11.73% | 147.80% | 100.0% | 6.28% |

**核心发现**：

- ✅ **理论正确**：预测概率越大，实际涨幅也越大
- ✅ **阶梯式增长**：从概率0.50到0.90，20天平均涨幅从-0.24%增长到11.73%，年化收益率从-3.02%增长到147.80%
- ✅ **胜率同步提升**：概率0.50-0.55胜率40.6%，概率>0.90胜率100%
- ✅ **高质量信号**：概率>0.70的信号，20天平均涨幅9.25%（年化116.55%），胜率93.9%
...
|------|--------|--------|--------|----------|----------|
| **CatBoost** | **60.88%** ⭐ | **0.6416** ⭐ | **2.06%** | 快（1-2分钟） | ⭐⭐⭐⭐⭐ |
...
| 20天 | GBDT | 57.94% | 0.7007 | 8.22% | ⭐⭐⭐ | ⬇️ -2.05% |
| 20天 | LightGBM | 58.93% | 0.7102 | 8.35% | ⭐⭐⭐ | ⬇️ -0.05% |
...
**关键发现**（添加市场环境自适应、风险管理特征、One-Hot编码后）：
- ✅ **CatBoost 20天模型**：准确率60.88%，F1分数0.6416，标准差2.06%
- ⚠️ **GBDT 20天略有下降**：准确率从59.99%降至57.94%（-2.05%），新特征对GBDT帮助有限
- ✅ **LightGBM 20天One-Hot编码优化**：准确率从58.98%提升至58.93%（+1.53%），F1分数0.7102
- 💡 **原因分析**：
  - CatBoost使用Ordered Target Statistics自动处理分类特征，无需One-Hot编码
  - LightGBM通过One-Hot编码处理Market_Regime分类特征，准确率提升1.53%
  - GBDT对编码方式不敏感，性能基本持平

**重要提示**：
- ⭐ **推荐使用 CatBoost 20天模型**：准确率60.88%，F1分数0.6416，标准差2.06%
...
| LightGBM 20天 | 58.93% | 0.7102 | 8.35% | ⭐⭐⭐ |
| GBDT 20天 | 57.94% | 0.7007 | 8.22% | ⭐⭐⭐ |
