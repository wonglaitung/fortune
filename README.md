...
- 完整的交易日志记录（按日期分割）
- 详细的持仓详情展示和每日总结功能
- **最新功能**：将大模型建议的买卖原因添加到所有通知邮件中，使用户能够更好地理解交易决策的依据
- **优化功能**：完全采用大模型建议的资金分配比例进行投资，系统不再自动计算投资比例，避免过度集中投资
- **优化功能**：大模型提示词已优化，去除买入股数字段，只保留资金分配比例字段，提高风险控制精度
- **优化功能**：删除了未使用的函数，如calculate_investment_percentage和get_current_positions_list，使代码更简洁
...
# 设置分析频率（默认每15分钟）
python simulation_trader.py --analysis-frequency 15
