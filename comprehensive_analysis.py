...
4. **风险控制建议**：
   ...
   **特别要求 - 考虑模型不确定性**：
   - ML 20天模型标准差为±6.33%（LightGBM）/±6.06%（GBDT+LR）
...
## 强烈买入信号（2-3只）
1. [股票代码] [股票名称] 
   - 推荐理由：[详细的推荐理由，必须说明：短期建议+中期建议+ML预测+一致性程度。例如："短期建议买入（触发器），中期建议买入（确认器），LightGBM预测上涨概率0.72，GBDT+LR预测上涨概率0.68，短中长期方向一致（短期/中期一致买入，ML模型验证上涨），综合置信度高。注意ML模型当前准确率57.63%（标准差±6.33%），probability在0.72附近实际准确率可能在58% ~ 66%之间"]
   - 操作建议：买入/卖出/持有/观望
   - 建议仓位：[X]%
   - 价格指引：
     * 建议买入价：HK$XX.XX
