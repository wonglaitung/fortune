...
**阈值优化说明**：
- 当前20天模型准确率：LightGBM 59.11%（标准差±6.90%），GBDT+LR 58.32%（标准差±7.31%）
...
**重要说明 - 模型不确定性**：
- ML 20天模型准确率：LightGBM 59.11%（标准差±6.90%），GBDT+LR 58.32%（标准差±7.31%）
- 即使probability>0.62，实际准确率也可能在52.21% ~ 66.01%（LightGBM）或51.01% ~ 65.63%（GBDT+LR）之间波动
...
4. **风险控制建议**：
   ...
   **特别要求 - 考虑模型不确定性**：
   - ML 20天模型标准差为±6.90%（LightGBM）/±7.31%（GBDT+LR）
...
## 强烈买入信号（2-3只）
1. [股票代码] [股票名称] 
   - 推荐理由：[详细的推荐理由，必须说明：短期建议+中期建议+ML预测+一致性程度。例如："短期建议买入（触发器），中期建议买入（确认器），LightGBM预测上涨概率0.72，GBDT+LR预测上涨概率0.68，短中长期方向一致（短期/中期一致买入，ML模型验证上涨），综合置信度高。注意ML模型当前准确率59.11%（标准差±6.90%），probability在0.72附近实际准确率可能在59% ~ 66%之间"]
   - 操作建议：买入/卖出/持有/观望
   - 建议仓位：[X]%
   - 价格指引：
     * 建议买入价：HK$XX.XX
