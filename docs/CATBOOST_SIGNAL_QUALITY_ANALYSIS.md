...
|------|------------|------------|---------|-------------|---------|
| **CatBoost 20天** | **238.76%** | **1.51** | **32.81%** | **86%（24/28）** | **±1.95%** |
| GBDT 20天 | -1.86% | -0.06 | 29.88% | 14%（4/28） | ±4.34% |
| LightGBM 20天 | -8.22% | -0.18 | 29.57% | 7%（2/28） | ±3.50% |

**关键发现**：
1. **CatBoost 表现卓越**：24只股票收益率>50%，平均收益率238.76%
2. **GBDT/LightGBM 表现不佳**：大部分股票收益率<0%，平均收益率-1.86%和-8.22%
3. **稳定性差异巨大**：CatBoost 标准偏差±1.95%，GBDT 标准偏差±4.34%（相差2.2倍）
...
**结果对比**：
- CatBoost：信号质量高，收益率 238.76%，优秀股票占比 86%
- GBDT：信号质量低，收益率 -1.86%，优秀股票占比 14%
- LightGBM：信号质量低，收益率 -8.22%，优秀股票占比 7%
...
**回测表现**：平均总收益率（基于2年回测）238.76%，优秀股票占比 86%