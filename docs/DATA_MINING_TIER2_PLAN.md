...
| 维度 | 状态 | 新增特征数 | 验证结果 | 备注 |
|------|------|-----------|---------|------|
| 2.1 Regime 增强 | ✅ 已验证 | 4/4 | **56.20%** (+0.61%) | 有效，已集成 |
| 2.2 跨尺度关联 | ⚠️ 待优化 | 5/5 | 50.76% (-4.79%) | 需重新设计 |
| 2.3 信息衰减 | ⏸️ 待实施 | 0/5 | - | 需先运行 MI 分析 |
| **Walk-forward 验证** | ✅ 已完成 | - | - | 基线 55.59% |
...
| 维度 | 状态 | 新增特征数 | 实际耗时 | 备注 |
|------|------|-----------|---------|------|
| 2.1 Regime 增强 | ⏸️ 待集成 | 0/4 | - | 复用恒指特征 |
| 2.2 跨尺度关联 | ⏸️ 待验证 | 0/5 | - | 需验证对个股有效性 |
...
| 类别 | 恒指当前 | 恒指 Tier 1 目标 | 个股当前 | 个股 Tier 1 目标 | 验证状态 |
|------|---------|-----------------|---------|-----------------|---------|
| Regime | 10 | 10 | 6 | 10 | ✅ 有效 (+0.61%) |
| 跨尺度 | 0 | 5 | 0 | 5 | ⚠️ 待优化 |
| 信息衰减 | 0 | 5 | 0 | 0-5 | ⏸️ 待实施 |
| 模式指纹 | 0 | 3 (Tier 2) | 0 | 不适用 | - |
| 市场结构 | 0 | 2 (Tier 2) | 0 | 不适用 | - |
| **合计** | **55** | **75** | **51** | **61-66** | - |

**验证结论**（2026-04-28）：
- 基线 Walk-forward 准确率：55.59%
- Regime 增强后：56.20%（+0.61%）
- 跨尺度特征需重新设计
...
- 文档写 `[-3,+3]` 但未解释各值含义
- 建议：补充说明（+3=三周期一致跑赢，-3=一致跑输；恒指用绝对标签：+3=一致看涨，-3=一致看跌）
