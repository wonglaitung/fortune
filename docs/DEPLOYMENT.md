...
2. **🟢 升级**（净IR≥0.7 且 PBO<0.5 且 DSR≥0.95）→ 可放大至 15–20%；
   ...
   **按"小幅升配"执行：先到 15%，勿一步到 20%**（n=39 期、2024 打平、信号层 lift 仍不显著）；
3. **🟡 保留** → 仅 ≤10% 倾斜；
4. **🔴 停用**（IR≤0）→ 清仓。
...
| 20d 中性 TopK 护栏 | **🟢 可升配（小幅）** | 净IR **1.41** [0.32,2.62] / PBO 0.19 / DSR 0.982；超额IR **1.24** [0.18,2.41]、逐年超额全正 → 统计全过，**执行升到 15%**（DECISIONS D2 + §四） |
