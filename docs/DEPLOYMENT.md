...
| T2 | **20d 行业中性 TopK** | 净IR **1.41** [0.32,2.62]、PBO 0.19、DSR 0.982、**超额IR 1.24 [0.18,2.41]**、逐年超额全正（2026-09-26） | 辅助倾斜 | 20 日 | **🟢 已达标 → 可放大至 15%** |
...
战术2 ≤15%    20d 中性 TopK（2026-09-26 护栏 🟢 + CI/逐年全过 → 小幅放大）
...
2. **🟢 升级**（净IR≥0.7 且 PBO<0.5 且 DSR≥0.95）→ 可放大至 15–20%；
   **2026-09-26 已 🟢**，且超额 CI [0.18,2.41] 下界>0、逐年超额全正 →
   **统计上够格升到 15%，是否执行由人工确认**；若执行也**勿一步到 20%**
   （n=39 期、2024 打平、信号层 lift 仍不显著）；
3. **🟡 保留** → 仅 ≤10% 倾斜；
4. **🔴 停用**（IR≤0）→ 清仓。
...
| 复核 | `monthly_guardrail.py` + `portfolio_backtest.py` | 每月：净IR/PBO/DSR/lift + **超额CI/逐年** |
...
| 20d 中性 TopK 护栏 | **🟢 达标，待人工确认升配** | 净IR **1.41** [0.32,2.62] / PBO 0.19 / DSR 0.982；超额IR **1.24** [0.18,2.41]、逐年超额全正 → 统计全过，按预设规则**可升至 15%**（是否执行由人工定，DECISIONS D2 + §四） |

**⚠️ 防误读**：
- 报告里**板块胜率**（如交易所 20d 70.6%）与**三周期模式**（011 探底回升 67.7%、101 假突破 67.4%）
  是**3 个月滚动、未 embargo、小样本**的实时统计，与 PIT 全期结论（八模式相对同向 20d 基准的
  净贡献 Bonferroni 后**全部不显著**，如 101 净 −1.6pp）矛盾，**不能当交易信号**；
...
- `20d 中性 TopK`：净IR / PBO / DSR 逐月监控，判定见 DECISIONS D2；
  **每次 walk-forward 后另跑 `portfolio_backtest.py`** 看超额 CI 与逐年（判定见
  [VALIDATION_GUIDE](VALIDATION_GUIDE.md) 月度护栏章"两道额外检验"）；
