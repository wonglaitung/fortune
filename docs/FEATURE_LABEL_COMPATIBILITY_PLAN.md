...
11. **损失函数对比**：
    | 指标 | YetiRank（基线） | YetiRankPairwise（当前） | 目标 | 状态 |
    |------|-----------------|--------------------------|------|------|
    | IC | +0.0122 | +0.0066 | >0.01 | ⚠️ 下降 |
    | Rank IC | -0.0060 | **+0.0038** | **>0.02** | ✅ 从负变正 |
...
15. **核心原则**：如果 Rank IC ≥ 0.015，且 ICIR 高、Sortino > 5.0，不要过度追求超参优化。港股非线性极强，过度调参往往是过拟合的开始。
