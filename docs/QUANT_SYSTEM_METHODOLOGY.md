...
| **想彻底搞懂为什么这么设计** | 全读，顺序：TL;DR 主线 → 每章"说人话版" → 细节 → 术语表 | 无 |

> 下面每一章的**第一行都是"🫖 说人话版"**，用 20d 的故事把本章讲成一句话；
> 看懂了就可以跳过该章细节，需要时再回来。
...
> 🫖 **说人话版**：别问"模型准不准"，问"敢不敢照着下单"。50.9% 的模型可能亏钱、49.6% 的模型可能赚钱
> ——差的就是这篇文档讲的门槛。
...
> 🫖 **说人话版**：判断一个策略像**剥洋葱**——数据掺没掺假 → 信号有没有真本事 → 扣了成本还赚不赚 →
> 敢下多大注 → 记没记进账本。20d 那个策略就是一层层剥下来，才到今天"15% 仓位"这个结论的。
...
> 🫖 **说人话版**：七条全是防被骗。最骗人的三条：
> ① 看"绝对准确率"（牛市里乱猜都对）；② 只看整体不看年份（可能全靠 2025 一年撑）；
> ③ 把"统计过关"当"可以满仓"（结果是弱信号只配 15%）。
...
> 🫖 **说人话版**：先花一周立骨架（口径、时点、闸门、测试），再开始做模型。
> 顺序反了就得返工——我们就是"先建模型、后补闸门"，每补一道重跑一次全量（约 3 小时）。
> **骨架先立好，后面全在省时间。**
...
> 🫖 **说人话版**：一个想法从"好像有用"到"能下单"走六步，任何一步不过就淘汰。
> 最值钱的是第 2 步——用最便宜的实验先杀掉明显的伪信号，省下未来 3 天。
...
> 🫖 **说人话版**：这些坑我们都踩过、记成黑名单了。你看到哪条眼熟，先停下手中的活——
> 你可能正在重复踩。
...
> 🫖 **说人话版**：统计负责"筛掉骗子"，你负责"下多大注"。
> 统计说"不显著"，你不能改口说"就是显著"——只能降仓或不用；
> 统计说"过关"，你还得决定 15% 还是 20%。
...
> 🫖 **说人话版**：上线前对这张表打勾，缺一项就不要发。三分钟能做完，缺了可能亏三个月。
...
> 🫖 **说人话版**：这是查字典的地方。正文里遇到不懂的词，大多是这 37 条之一。
