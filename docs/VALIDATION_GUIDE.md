...
> **最后更新**：2026-09-27（D3/D8 口径修正 + 旧结论存档横幅；六周期 walk-forward 复测为 2026-09-26）
...
> ⚠️ **历史存档（2026-04 绝对口径），"强烈推荐实盘"结论不可引用**：本例按绝对准确率/夏普
> 评级，已被 **D3 禁用绝对准确率** 推翻——银行 20d 准确率 58.3% 全场最高，但"永远看涨"
> 基准 62.4% → **方向技能 −4.2pp**（见下方"关键判读"）。现行只看 lift / 方向技能。
...
### 个股模型 Walk-forward 验证示例（2026-05-09）

> ⚠️ **历史存档（2026-05 绝对口径）**："综合评分 90 / 强烈推荐"按绝对准确率与毛收益评级，
> 现行评估不用该口径（D3）；IC 0.21 亦为 59 池全量旧样本，与 2026-09 PIT 复测不可比。
...
**关键发现**：
- ✅ 夏普比率 0.9153，达标业界标准 >0.9
- ✅ 最大回撤极低（-0.21%），风险控制优秀
- ✅ 索提诺比率 4.14，远超业界标准
- ⚠️ 准确率 55.70%，未达目标 >58%
- ⚠️ 稳定性评级低，收益率波动大（标准差 15.47%）
...
> ⚠️ **历史存档（2026-04 绝对胜率/夏普口径）**：下表"推荐度/强烈推荐实盘"按绝对数字排名，
> 已被 D3 推翻（趋势板块"永远看涨"胜率本就高）。板块层现行口径：板块 lift 跨周期
> Spearman(5d,20d)=0.73 **可迁移用于倾斜**，但不作绝对推荐（见"关键判读"）。
...
> ⚠️ **已被推翻（2026-09-26）**：本节用**无 embargo 的日级回测**（938 样本、16 特征、2020-2024），
> 得出"策略有效性 ✅ 验证通过""建议保持策略"。PIT/embargo 复测后，八大模式相对同向 20d
> 基准的净贡献 **Bonferroni 校正后全部不显著**，"至少一周期正确率 92%"是**把 3 个二元事件
> 任一对即算对**的口径膨胀，不构成预测优势。**本节仅作历史存档，不可作为交易依据。**
> 现行结论见本文[最新验证结果](#最新验证结果)与 [THREE_HORIZON_ANALYSIS.md](THREE_HORIZON_ANALYSIS.md)。
...
| 指标 | 数值 | 说明 |
|------|------|------|
| 平均收益率 | 19.74% | 12个Fold平均 |
| 收益率标准差 | 15.47% | 波动较大 |
| 收益率范围 | 43.64% | 最高47.32% - 最低4.49% |
| 稳定性评级 | 低（需改进） | 标准差 > 10% |
...
| 指标 | 数值 | 说明 |
|------|------|------|
| 平均夏普比率 | 0.9153 | 达标业界标准0.9 |
| 夏普标准差 | 0.4263 | 波动中等 |
| 夏普范围 | 1.77 | 最高1.87 - 最低0.10 |
...
| 平均准确率 | 55.70% | 略高于随机 |
| 准确率标准差 | 9.06% | 波动较大 |
| 准确率范围 | 24.37% | 最高67.81% - 最低43.99% |

### 关键发现

1. **夏普比率达标**：平均夏普比率0.9153，达标业界标准>0.9
2. **最大回撤极低**：平均-0.21%，风险控制优秀
3. **稳定性待提升**：收益率标准差15.47%，波动较大
4. **季度效应明显**：Q2收益最高，Q3夏普最高，Q4准确率最高但收益低
5. **准确率与收益非线性**：高准确率Fold收益转化需优化

### 优化建议

> ⚠️ **历史存档（2026-04-29 回测），部分建议已被证伪**：
> ①"训练窗口增至 18 个月" ❌ **lessons 三.3 证伪**（12m 60.1% > 18m 56.6% > 24m 54.9%——
> 训练窗口不是越大越好）；②"优化置信度阈值"属调参类，须先过 D1/D3 护栏再谈。
