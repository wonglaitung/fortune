...
> **最后更新**：2026-09-26（六周期 walk-forward 全量复测后）
...
| 个股 | 20d LightGBM | 51.8% [49.7,53.9] p=0.105 | +1.9pp p=0.240 | 净IR 1.41 [0.32,2.62] PBO 0.19 DSR 0.982 🟢 | 护栏过线但 lift 不显著，升级需人工决策 |
...
**买入信号集** = 市场情绪过滤后 `p >= Dynamic_Threshold`（0.5/0.65/0.7/1.0）；**全样本** = 所有已验证预测。
