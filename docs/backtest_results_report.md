...
|------|------|------|
| 回测股票数量 | 28 只 | 全部自选股 |
| 平均总收益率（基于2年回测） | **133.87%** 🎯 | **+16.3% 提升** ⭐ |
| 年化收益率 | **57.3%** | 相比旧模型 54.9% |
| 平均夏普比率 | **1.11** | **+11% 提升** ⭐ |
| 平均最大回撤 | **-21.51%** | **风险控制改善** ⭐ |
| 平均买入信号胜率 | **32.01%** | **+0.12% 提升** ⭐ |

### 收益分布（最新融合模型）

| 指标 | 数值 | 备注 |
|------|------|------|
| 最高收益率（基于2年回测） | **495.68%** | **+32.3% 提升** ⭐ |
| 最低收益率 | -36.53% | 1211.HK（比亚迪股份） |
| 收益率中位数（基于2年回测） | **94.06%** | **+23.9% 提升** ⭐ |
| 收益率标准差 | - | 详细数据见对比 |
...
|------|-----------|-----------|------|------|
| 平均总收益率（基于2年回测） | 115.13% | **133.87%** | **+18.74%** | **+16.3%** ⭐ |
| 年化收益率 | 54.9% | **57.3%** | **+2.4%** | **+2.4%** ⭐ |
| 平均夏普比率 | 1.00 | **1.11** | **+0.11** | **+11%** ⭐ |
| 平均最大回撤 | -23.07% | **-21.51%** | **+1.56%** | **风险改善** ⭐ |
| 收益率中位数（基于2年回测） | 75.93% | **94.06%** | **+18.13%** | **+23.9%** ⭐ |
| 最高收益率（基于2年回测） | 374.67% | **495.68%** | **+121.01%** | **+32.3%** ⭐ |

---

## Top 10 股票表现（新融合模型）

| 排名 | 股票代码 | 股票名称 | 总收益率（基于2年回测） | 年化收益率 | 夏普比率 | 最大回撤 | 买入信号胜率 |
|------|---------|---------|---------|-----------|---------|---------|------|
...
| 股票 | 总收益率（基于2年回测） | 年化收益率 | 夏普比率 | 最大回撤 | 买入信号胜率 |
|------|---------|-----------|---------|---------|------|
...
🏆 **最佳表现**:
1. **1347.HK 华虹半导体**: 495.68% 总收益率（基于2年回测），1.52 夏普比率 ⭐
2. **0981.HK 中芯国际**: 328.47% 总收益率（基于2年回测），1.32 夏普比率
3. **1810.HK 小米集团-W**: 285.62% 总收益率（基于2年回测），1.81 夏普比率
4. **9660.HK 地平线机器人**: 256.43% 总收益率（基于2年回测），2.15 夏普比率 ⭐

### 2. 行业表现对比

| 行业 | 平均收益率（基于2年回测） | 平均夏普 | 特点 |
|------|-----------|---------|------|
...
✅ **优点**:
- 平均收益率（基于2年回测） **133.87%**，相比旧模型 **+16.3%** ⭐
...
2. ✅ **训练速度提升**: 20-32%（显著提升）
3. ✅ **预测性能**: 平均收益率（基于2年回测） 133.87% （+16.3% 提升）⭐
4. ✅ **稳定性提升**: 减少过拟合风险
...
**推荐股票**:
1. **9660.HK 地平线**: 夏普 2.15，回撤 -16.04%，收益（基于2年回测） 256.43% ⭐⭐⭐
2. **1810.HK 小米**: 夏普 1.81，回撤 -27.88%，收益（基于2年回测） 285.62% ⭐⭐⭐
3. **1347.HK 华虹**: 夏普 1.52，回撤 -20.62%，收益（基于2年回测） 495.68% ⭐⭐
4. **0981.HK 中芯**: 夏普 1.32，回撤 -19.28%，收益（基于2年回测） 328.47% ⭐⭐
...
| 最高收益率（基于2年回测） | 374.67% | **495.68%** | **+32.3%** | ⭐⭐⭐⭐ |
...
**回测结果**: ✅ 成功，平均收益率（基于2年回测） **133.87%**（+16.3% 提升）⭐
