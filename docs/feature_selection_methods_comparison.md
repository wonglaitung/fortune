...
| 平均买入信号胜率 | 31.89% | **32.01%** | **+0.12%** |
| 最高收益率（基于2年回测） | 374.67% | **495.68%** | **+32.3%** |
| 收益率中位数（基于2年回测） | 75.93% | **94.06%** | **+23.9%** |
...
| 模型 | 平均总收益率（基于2年回测） | 年化收益率 | 平均夏普比率 | 平均最大回撤 | 平均买入信号胜率 |
|------|-------------|-------------|-------------|-------------|----------|
...
- **预期平均收益率（基于2年回测）**: 133.87%
- **预期年化收益率**: 57.3%
- **预期平均夏普比率**: 1.11
- **预期平均最大回撤**: -21.51%

**优点**：
- ✅ 平均收益率（基于2年回测）提升 **16.3%** (115.13% → 133.87%)
- ✅ 年化收益率提升 **2.4%** (54.9% → 57.3%)
- ✅ 夏普比率提升 **11%** (1.00 → 1.11)
...
2. **验证回测结果**：
   - 融合模型平均收益率（基于2年回测）: **133.87%** (vs 旧模型 115.13%)
   - 融合模型年化收益率: **57.3%** (vs 旧模型 54.9%)
...
**理由**：
1. ✅ 平均收益率（基于2年回测）提升 **16.3%** (115.13% → 133.87%)
2. ✅ 年化收益率提升 **2.4%** (54.9% → 57.3%)
...
### 融合模型显著改进
- **收益率（基于2年回测）提升**: 115.13% → 133.87% (+16.3%)
- **年化收益率提升**: 54.9% → 57.3% (+2.4%)
- **夏普比率提升**: 1.00 → 1.11 (+11%)
- **风险控制改善**: 最大回撤从-23.07% → -21.51%
- **稳定性提升**: 在更多股票上实现高收益

### CatBoost模型改进
- **收益率（基于2年回测）提升**: 238.76% → 241.23% (+1.0%)
- **年化收益率提升**: 88.2% → 88.5% (+0.3%)
...
**最新回测数据**：融合模型平均收益率（基于2年回测） 133.87%，年化收益率 57.3%