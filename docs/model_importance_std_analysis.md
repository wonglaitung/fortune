...
| 预测周期 | 推荐特征集 | 准确率 | 标准差 | CV | 速度提升 | 综合评分 | **回测收益（20天周期）** |
|---------|-----------|--------|--------|-----|---------|---------|----------------------|
...
| 预测周期 | 特征集 | 准确率 | 标准差 | CV | 速度提升 | **回测收益（20天周期）** | 稳定性 |
|---------|--------|--------|--------|-----|---------|----------------------|--------|
| **20天** | **模型重要性法500特征** | **~61-62%** | **~2%** | **~3%** | **+20-32%** | **133.87%** | ⭐⭐⭐⭐⭐ |

**注意**：必须标注收益率周期，避免混淆：
- **错误**：收益率133.87%（未标注周期）
- **正确**：平均总收益率（20天周期）133.87%

---

**报告生成时间**：2026-02-24
**数据来源**：训练日志交叉验证结果，批量回测结果
**最新改进**：融合模型平均收益率（20天周期）133.87%，相比旧模型+16.3%