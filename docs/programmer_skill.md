...
### 机器学习模型的生命线：
训练时和预测时的特征计算逻辑**必须完全一致**，否则模型将无法正确预测。

### 常见不一致问题：

| 问题类型 | 训练时 | 预测时 | 后果 |
|---------|--------|--------|------|
| **网络特征** | 实时计算 | 默认值 (0/-1) | 特征重要性极低 |
| **截面特征** | 实时计算 | 默认值/回退值 | 预测精度下降 |
| **特征顺序** | 顺序A | 顺序B | 特征错位，预测错误 |
| **缺失值处理** | 填充均值 | 填充0 | 分布不一致 |

### 检查清单：
- [ ] **特征计算逻辑**：训练和预测使用相同的函数
- [ ] **特征顺序**：确保特征列顺序一致
- [ ] **缺失值处理**：训练和预测使用相同的填充策略
- [ ] **分类特征编码**：使用训练时的编码器
- [ ] **归一化/标准化**：使用训练时的统计量

### 批量预测优先：
对于需要跨截面数据的特征（如网络特征、截面百分位特征），**必须使用 `predict_batch()`**，而非单股预测 `predict()`。

### 核心理念：
**训练和预测的不一致是机器学习模型的隐形杀手。任何特征工程的修改，都必须同时验证训练和预测两个路径。**
