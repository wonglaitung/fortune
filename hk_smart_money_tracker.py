...
格式要求：
- 必须包含：[操作类型] [股票代码] [股票名称] | 综合评分:XX分 | 建议仓位:XX% | 目标价:XX | 止损价:XX | 持仓时间:XX | 风险等级:X级 | 买入理由:XXXX | 风险因素:XXXX
- 使用"|"作为字段分隔符
- 操作类型必须使用方括号：[BUY]、[SELL]、[HOLD]、[CLOSE]
- 股票代码必须使用标准格式：0005.HK、0388.HK等
- 每个字段之间用" | "分隔（注意：分隔符前后有空格）
