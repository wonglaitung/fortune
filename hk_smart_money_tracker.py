...
- **多周期指标**：提供3/5/10/20/60日多时间维度的价格变化率、趋势方向和相对强度分析，帮助识别不同时间尺度的市场趋势和股票相对表现
  - **收益率(%)**：指定周期内的价格变化率，正值表示上涨，负值表示下跌
  - **趋势**：基于收益率判断的趋势方向（强势上涨、上涨、下跌、强势下跌）
  - **相对强度(%)**：股票收益率相对于恒生指数收益率的差值，正值表示跑赢恒指，负值表示跑输恒指
  - **相对强度信号**：基于相对强度判断的信号（显著跑赢、跑赢、持平、跑输、显著跑输）
  - **多周期趋势评分**：综合多个周期的趋势评分（-100到100），正值表示多头趋势，负值表示空头趋势
  - **多周期相对强度评分**：综合多个周期的相对强度评分（-100到100），正值表示整体跑赢恒指，负值表示整体跑输恒指
...
- **多周期趋势分析**：
  - 分析3/5/10/20/60日收益率和趋势方向，识别短期到中长期的市场趋势
  - 关注多周期趋势评分，正值表示整体多头趋势，负值表示整体空头趋势
  - 识别趋势一致性（如3/5/10/20/60日均为上涨趋势表示强势多头）
  - 分析趋势转换信号（如短期上涨但中长期下跌可能表示反弹而非反转）
- **多周期相对强度分析**：
  - 分析3/5/10/20/60日相对强度，识别股票在不同时间尺度上跑赢/跑输恒指的情况
  - 关注多周期相对强度评分，正值表示整体跑赢恒指，负值表示整体跑输恒指
  - 识别相对强度改善信号（如短期相对强度显著跑赢且中长期相对强度改善）
  - 分析相对强度与趋势的协同性（如上涨趋势且相对强度跑赢表示强势上涨）
