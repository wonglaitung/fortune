...
【第六步：新闻信息影响评估】
📰 新闻影响：
- 正面新闻配合建仓评分高和基本面扎实：强烈推荐
- 负面新闻配合出货评分高或基本面薄弱：强烈规避
- 新闻情绪与技术指标背离：谨慎对待，等待确认
...
1. 🎯 综合推荐（3-5只，按综合评分排序）
   - 股票代码及名称
   - 综合评分：XX分（详细说明各维度得分）
   - 建仓评分：XX分（建仓级别：strong/partial/none）
   - 多周期趋势评分：XX分（趋势方向：强势多头/多头/震荡/空头/强势空头）
   - 多周期相对强度评分：XX分（相对强度：显著跑赢/跑赢/持平/跑输/显著跑输）
   - 基本面评分：XX分（估值：PE=XX，PB=XX）
   - 技术指标亮点：RSI=XX，MACD=XX，TAV评分=XX，成交量比率=XX
   - 新闻影响评估：正面/负面/中性（如有相关新闻）
   - 推荐理由：基于综合评分的详细分析
   - 建议仓位：XX%（根据综合评分和市场环境调整）
   - 风险控制建议：如有触发止盈/止损/Trailing Stop，给出具体操作

2. ⚠️ 风险警示（按风险等级排序）
   - 🔴 极高风险（止损触发）：股票代码及名称，止损理由，应对建议（全部卖出）
   - 🟡 高风险（止盈/Trailing Stop触发）：股票代码及名称，风险理由，应对建议（卖出30%）
   - 🟠 中风险（出货评分高或基本面差）：股票代码及名称，风险来源，应对建议（减仓30-60%）
   - 🟡 低风险（技术面恶化）：股票代码及名称，风险来源，应对建议（观望或减仓）

3. 📊 市场总结
   - 市场环境评估：情绪、资金流向、整体强度
   - 技术面强度：建仓信号X只，出货信号Y只，建仓/出货比例
   - 基本面质量：高评分股票X只，占比XX%
   - 风险控制统计：止损X只，止盈Y只，Trailing Stop Z只
   - 新闻面影响：正面新闻X条，负面新闻Y条
   - 投资策略建议：根据市场环境给出整体策略（积极/中性/谨慎）
   - 仓位管理建议：建议整体仓位XX%（根据市场环境调整）

报告生成时间：{current_time}，数据截至：{run_date if run_date else '最新数据'}

⚠️ 重要提醒：
1. 风险控制信号（止损/止盈/Trailing Stop）必须优先处理，不可忽视
2. 综合评分是推荐的核心依据，必须详细说明各维度得分
3. 市场环境必须考虑，根据市场情绪和资金流向调整策略
