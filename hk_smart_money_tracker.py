...
【交易建议清单】
⚠️ 重要：每条建议必须严格按照以下格式输出，不能遗漏任何字段！

格式要求：
- 每条建议必须以操作类型开头：[BUY] 或 [SELL] 或 [HOLD] 或 [CLOSE]
- 格式：[操作类型] 股票代码 股票名称 | 综合评分:XX分 | 建议仓位:XX% | 目标价:XX | 止损价:XX | 持仓时间:XX | 风险等级:X级 | 买入理由:XXXX | 风险因素:XXXX
- 所有字段之间用" | "分隔（注意：分隔符前后有空格）
- 股票代码格式：0005.HK、0388.HK等（必须包含.HK后缀）
...
[HOLD] 0700.HK 腾讯控股 | 综合评分:45分 | 建议仓位:20% | 目标价:N/A | 止损价:N/A | 持仓时间:短期 | 风险等级:3级 | 买入理由:N/A | 风险因素:建仓评分和出货评分接近，多周期趋势震荡，建议观望
