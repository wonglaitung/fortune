...
2. **出货信号优先**：
   - 出货评分≥5.0（strong）：SELL（卖出60-100%），风险等级4级
   - 出货评分3.0-5.0（weak）：SELL（卖出30-60%），风险等级3级

3. **建仓信号评估**：
   - 综合评分>70分且建仓评分≥5.0：BUY（买入50-70%），风险等级1-2级
   - 综合评分50-70分且建仓评分≥3.0：BUY（买入30-50%），风险等级2-3级
   - 综合评分30-50分且建仓评分≥3.0：HOLD（持有），风险等级3级
...
6. **风险等级评估**：
   - 1级（低风险）：综合评分>70，基本面评分>60，多周期趋势评分>20，无风险控制信号
   - 2级（中低风险）：综合评分50-70，基本面评分>40，多周期趋势评分>0，无风险控制信号
   - 3级（中风险）：综合评分30-50，基本面评分>20，多周期趋势评分>-20，无风险控制信号
   - 4级（中高风险）：出货评分≥3.0或触发止盈/Trailing Stop，或综合评分<30
   - 5级（高风险）：触发止损，或出货评分≥5.0且基本面评分<40

📈 输出格式（严格按此格式输出，便于模拟交易程序解析）：

【交易建议清单】
格式：[操作类型] [股票代码] [股票名称] | 综合评分:XX分 | 建议仓位:XX% | 目标价:XX | 止损价:XX | 持仓时间:XX | 风险等级:X级 | 买入理由:XXXX | 风险因素:XXXX
...
示例：
- [BUY] 0939.HK 建设银行 | 综合评分:75分 | 建议仓位:50% | 目标价:8.50 | 止损价:7.00 | 持仓时间:中期 | 风险等级:2级 | 买入理由:建仓评分5.2(strong)，多周期趋势评分40，基本面评分70，技术指标协同良好 | 风险因素:市场情绪中性
- [SELL] 3968.HK 招商银行 | 综合评分:35分 | 建议仓位:0% | 目标价:N/A | 止损价:N/A | 持仓时间:N/A | 风险等级:4级 | 买入理由:N/A | 风险因素:出货评分5.0(strong)，多周期趋势评分-20，基本面评分40，技术指标协同恶化
