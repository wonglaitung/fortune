...
多周期预测验证结果（Walk-forward，2026-04-28 更新，增强模型33特征）：
┌──────────┬──────────┬─────────────────────────────────────────────────┐
│   周期   │ 准确率   │                   策略建议                     │
├──────────┼──────────┼─────────────────────────────────────────────────┤
│   1天    │  49.67%  │ 噪音大，仅供参考                               │
│   5天    │  62.36%  │ 趋势确认，辅助判断                             │
│   20天   │  81.24%  │ 最可靠，主要决策依据                           │
└──────────┴──────────┴─────────────────────────────────────────────────┘
...
│     法则     │        条件         │   操作   │   胜率   │    备注     │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 假突破抄底⭐ │ 短涨中跌长涨(101)   │ 买入     │  95.00%  │   最优策略  │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 反弹失败⭐   │ 短跌中涨长跌(010)   │ 谨慎做多 │  85.98%  │   次优策略  │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 下跌中继     │ 短跌中跌长涨(001)   │ 谨慎做多 │  84.00%  │   样本充足  │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 一致看涨     │ 三周期一致看涨(111) │ 买入     │  80.62%  │   可靠      │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 趋势惯性     │ 三周期一致看跌(000) │ 减仓/做空│  79.57%  │   样本最多  │
└──────────────┴─────────────────────┴──────────┴──────────┴─────────────┘

关键发现：
- 假突破模式(101)是最优策略，胜率95.00%
- 反弹失败模式(010)胜率85.98%，次优策略
- 下跌中继模式(001)胜率84.00%，样本充足（100次）
- 一致看涨(111)准确率回升至80.62%
...
"""
    ...
    def _run_multi_horizon_prediction(self):
        ...
        try:
            ...
            # 更新时间：2026-05-13（恒指增强模型：33特征）
            historical_accuracy = {
                1: 0.4950,   # 49.50%（噪音大，仅供参考）
                5: 0.6077,   # 60.77%（趋势确认）
                20: 0.8077   # 80.77%（最可靠）
            }
