...
多周期预测验证结果（Walk-forward，2026-04-26 更新，增强模型33特征）：
┌──────────┬──────────┬─────────────────────────────────────────────────┐
│   周期   │ 准确率   │                   策略建议                     │
├──────────┼──────────┼─────────────────────────────────────────────────┤
│   1天    │  50.77%  │ 噪音大，仅供参考                               │
│   5天    │  58.28%  │ 趋势确认，辅助判断                             │
│   20天   │  80.24%  │ 最可靠，主要决策依据                           │
└──────────┴──────────┴─────────────────────────────────────────────────┘
...
│     法则     │        条件         │   操作   │   胜率   │    备注     │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 假突破抄底⭐ │ 短涨中跌长涨(101)   │ 买入     │  94.03%  │   最优策略  │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 下跌中继     │ 短跌中跌长涨(001)   │ 谨慎做多 │  86.24%  │   样本充足  │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 趋势惯性     │ 三周期一致看跌(000) │ 减仓/做空│  77.90%  │   样本最多  │
├──────────────┼─────────────────────┼──────────┼──────────┼─────────────┤
│ 一致追涨     │ 三周期一致看涨(111) │ 买入     │  70.00%  │   需谨慎    │
└──────────────┴─────────────────────┴──────────┴──────────┴─────────────┘

关键发现：
- 假突破模式(101)是最优策略，胜率94.03%
- 下跌中继模式(001)胜率86.24%，样本充足（109次）
- 一致信号准确率下降，一致看涨(111)从81.40%降至70.00%
