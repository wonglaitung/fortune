...
## 📊 预测性能监控经验 ⭐ 新增（2026-04-12）

### target_date 自动计算

**问题**：`prediction_history.json` 中预测记录缺少 `target_date` 字段，导致性能监控无法评估预测。

**解决方案**：
1. **保存时计算**：`save_prediction_to_history()` 自动根据 `data_date + horizon` 计算 `target_date`
2. **评估时补算**：`evaluate_predictions()` 如果缺少 `target_date`，动态计算
...
**概率（probability）** = 预测上涨的概率（`proba[1]`）

| 置信度 | 概率范围 | 预测方向 | 含义 |
|--------|---------|---------|------|
| **high** | > 0.6 | up | 强烈看涨 |
| **medium** | 0.5 ~ 0.6 | up | 温和看涨 |
| **low** | ≤ 0.5 | down | 看跌 |

**验证数据**：
- high 置信度：136 个预测，全部预测 up（概率范围 0.60~0.83）
- medium 置信度：124 个预测，全部预测 up（概率范围 0.50~0.60）
- low 置信度：141 个预测，全部预测 down（概率范围 0.24~0.50）
...
2. **预测下跌准确率低的原因**：
   - 市场整体上涨时，预测下跌失效
   - 24.11% 准确率意味着 75.89% 的预测下跌股票实际上涨

3. **交易策略建议**：
   - 优先使用 high/medium 置信度信号（即预测上涨信号）
   - 避免使用 low 置信度信号（预测下跌信号在牛市失效）
   - 可考虑设置置信度阈值 0.6（只使用 high 信号）
