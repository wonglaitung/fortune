...
## 📊 ML模型预测监控最佳实践 ⭐ 新增（2026-04-12）

### 1. 预测记录完整性

#### 问题
预测历史记录缺少 `target_date`（预测目标日期），导致性能监控无法评估预测。

#### 业界标准
预测记录应包含完整的时序信息：
- `data_date`：数据截止日期（预测输入）
- `target_date`：预测目标日期（预测输出）
- `horizon`：预测周期（天数）

#### 解决方案
```python
def get_target_date(data_date, horizon):
    """计算预测目标日期（业界标准：交易日 vs 自然日）
    
    注意：
    - 简单实现：data_date + horizon 天（自然日）
    - 精确实现：data_date + horizon 个交易日（需要交易日历）
    """
...
#### 最佳实践
1. **保存时计算**：在 `save_prediction_to_history()` 自动计算并保存
2. **评估时补算**：在 `evaluate_predictions()` 兼容缺失字段（向后兼容）
3. **数据一致性**：确保 `target_date` 与 `data_date + horizon` 一致
...
#### 业界定义
**预测概率（prediction_probability）** = P(上涨)，即模型预测上涨的概率。

| 概率值 | 预测方向 | 含义 |
|--------|---------|------|
| > 0.5 | up | 模型认为上涨概率 > 50% |
| ≤ 0.5 | down | 模型认为下跌概率 ≥ 50% |

#### 置信度分层（业界标准）
| 置信度 | 概率阈值 | 预测方向 | 适用场景 |
|--------|---------|---------|---------|
| **high** | P > 0.6 | up | 高确信度买入信号 |
| **medium** | 0.5 < P ≤ 0.6 | up | 中等确信度买入信号 |
| **low** | P ≤ 0.5 | down | 低确信度（方向不明确） |

#### 关键洞察
- **置信度与预测方向一一对应**：high/medium = up，low = down
- **不存在交叉**：没有 high 置信度预测 down 的情况
- **阈值选择**：0.5 是方向分界点，0.6 是高确信度阈值
...
#### 业界标准
未到期预测可使用当前价格计算**临时性能指标**，用于策略调整参考。

#### 数据验证（2026-03-25 至 2026-04-12，10 个交易日）
| 置信度 | 样本数 | 预测方向 | 准确率 | 平均收益 | 风险评估 |
|--------|--------|---------|--------|----------|---------|
| high | 136 | up | **84.56%** | +3.14% | 低风险 |
| medium | 124 | up | **80.65%** | +3.53% | 中低风险 |
| low | 141 | down | 24.11% | +2.83% | 高风险（市场上涨） |
...
#### 关键发现
1. **预测上涨信号有效**：high/medium 置信度准确率 > 80%
2. **预测下跌信号失效**：low 置信度准确率仅 24.11%（市场上涨环境）
3. **市场环境影响**：牛市中预测下跌信号准确率显著下降
...
| 策略 | 准确率 | 样本数 | 适用场景 |
|------|--------|--------|---------|
| **仅使用 high 信号** | 84.56% | 136 | 保守策略，追求高胜率 |
| **使用 high + medium** | 82.69% | 260 | 平衡策略，增加交易机会 |
| 避免使用 low 信号 | 24.11% | 141 | 牛市中预测下跌失效 |

#### 阈值设置建议
```python
# 推荐：只使用高确信度信号
CONFIDENCE_THRESHOLD = 0.6  # high 置信度阈值

# 过滤信号
def filter_signals(predictions, threshold=0.6):
    """只保留概率 > threshold 的预测"""
    return [p for p in predictions if p['probability'] > threshold]
```

#### 市场环境适配
- **牛市**：避免使用预测下跌信号（low 置信度）
- **熊市**：预测下跌信号可能更有效（需验证）
- **震荡市**：建议使用更严格的阈值（如 0.65）
...
| 指标 | 定义 | 说明 |
|------|------|------|
| 平均收益率 | 所有预测的平均收益 | 策略预期收益 |
| 夏普比率 | 收益 / 波动率 | 风险调整收益 |
| 最大回撤 | 峰值到谷值的最大跌幅 | 风险控制 |

#### 注意事项
- **中期指标仅供参考**：未到期预测的准确率可能变化
- **最终评估在到期后**：完整周期（20 交易日）后才能得出最终结论
- **持续监控**：定期更新指标，及时调整策略
...
| 维度 | 最佳实践 |
|------|---------|
| **数据完整性** | 预测记录包含 data_date、target_date、horizon |
| **置信度设计** | 基于概率阈值分层，与预测方向一一对应 |
| **信号过滤** | 根据置信度阈值过滤信号，提高胜率 |
| **市场适配** | 根据市场环境调整策略（牛市避免做空信号） |
| **持续监控** | 定期评估中期指标，及时调整策略 |
