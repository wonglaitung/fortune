...
|------|------------|------------|------------|---------|---------------------|------|
| **CatBoost 20天** | **206.72%** | **1.52** | -17.29% | **31.84%** | **28只（100%）** | ⭐⭐⭐⭐⭐ 最佳 |
| 融合模型（加权平均） | 140.69% | 1.01 | -28.61% | 29.20% | 28只（100%） | ⭐⭐⭐⭐⭐ 推荐 |
| GBDT 20天 | -1.63% | 0.10 | -42.64% | 29.13% | 5只（18%） | ⭐⭐⭐ 一般 |
| LightGBM 20天 | -1.85% | 0.10 | -41.75% | 28.63% | 4只（14%） | ⭐⭐ 较差 |

### 批量回测结果（置信度0.55，28只股票，2026-02-22）

| 模型 | 平均总收益率 | 平均夏普比率 | 平均最大回撤 | 平均胜率 | 优秀股票（收益率>50%） | 建议 |
|------|------------|------------|------------|---------|---------------------|------|
| **CatBoost 20天** | **238.76%** | **1.51** | -19.08% | **32.81%** | **24只（86%）** | ⭐⭐⭐⭐⭐ 最佳 |
| 融合模型（加权平均） | 115.13% | 1.00 | -26.81% | 31.89% | 22只（79%） | ⭐⭐⭐⭐⭐ 推荐 |
| GBDT 20天 | -1.86% | -0.06 | -43.10% | 29.88% | 4只（14%） | ⭐⭐⭐ 一般 |
| LightGBM 20天 | -8.22% | -0.18 | -43.25% | 29.57% | 2只（7%） | ⭐⭐ 较差 |

### 批量回测结果（置信度0.60，28只股票，2026-02-22）

| 模型 | 平均总收益率 | 平均夏普比率 | 平均最大回撤 | 平均胜率 | 优秀股票（收益率>50%） | 建议 |
|------|------------|------------|------------|---------|---------------------|------|
| **CatBoost 20天** | **206.72%** | **1.52** | -19.08% | **31.84%** | **23只（82%）** | ⭐⭐⭐⭐⭐ 最佳 |
| 融合模型（加权平均） | 75.97% | 0.86 | -24.61% | 30.97% | 15只（54%） | ⭐⭐⭐⭐ 推荐 |
| GBDT 20天 | -13.02% | -0.31 | -44.95% | 25.11% | 1只（4%） | ⭐⭐ 较差 |
| LightGBM 20天 | -14.96% | -0.24 | -44.85% | 26.47% | 0只（0%） | ⭐ 较差 |
...
1. **置信度0.50的显著优势**：
   - ✅ CatBoost和融合模型都实现100%成功率（所有股票都盈利）
   - ✅ 融合模型收益率最高（140.69%），比置信度0.55高出25.56%
   - ✅ CatBoost收益率稳定（206.72%），风险控制最佳（最大回撤-17.29%）

2. **置信度0.55的CatBoost表现最佳**：
   - ✅ CatBoost收益率最高（238.76%），比置信度0.50高出32.04%
   - ✅ CatBoost胜率最高（32.81%），比置信度0.50高出0.97%
   - ❌ 融合模型收益率下降（从140.69%降至115.13%）

3. **置信度0.60的保守特征**：
   - ✅ 交易频率最低（约30%），交易成本最少
   - ❌ CatBoost收益率下降（从238.76%降至206.72%）
   - ❌ 融合模型收益率大幅下降（从140.69%降至75.97%）

4. **为什么提高置信度不一定提高胜率和收益率？**
