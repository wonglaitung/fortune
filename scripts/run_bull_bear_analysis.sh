...
# 牛熊市分析自动化脚本
# 用法：
#   ./run_bull_bear_analysis.sh                    # 使用默认参数
#   ./run_bull_bear_analysis.sh 2024-01-01 2025-12-31    # 自定义日期范围
#   ./run_bull_bear_analysis.sh 2024-01-01 2025-12-31 csv    # 自定义输出格式
...
echo ""
echo "查看报告："
echo "  CSV: $(ls -t ${OUTPUT_DIR}/bull_bear_analysis_*.csv 2>/dev/null | head -1)"
echo "  JSON: $(ls -t ${OUTPUT_DIR}/bull_bear_analysis_*.json 2>/dev/null | head -1)"
echo "  Markdown: $(ls -t ${OUTPUT_DIR}/bull_bear_analysis_*.md 2>/dev/null | head -1)"