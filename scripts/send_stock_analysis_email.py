...
## 异常检测
30. **anomaly_alert**: 是否有异常检测警报（是/否）
31. **anomaly_reason**: 异常原因（如有）

## 股息信息
32. **dividend_info**: 股息信息（如"即将除净：2026-05-25，每股分红0.5元"或 null）

## 市场环境信息（所有股票共用，从报告开头提取）
33. **hsi_price**: 恒生指数价格（数值）
34. **hsi_change**: 恒生指数涨跌幅（百分比数值）
35. **market_status**: 市场状态（牛市/震荡偏涨/震荡市/震荡偏跌/熊市）
36. **market_duration**: 市场状态持续天数（数值）
37. **market_stability**: 市场状态稳定性（稳定/中等稳定/不稳定）
38. **vix**: VIX指数（数值）
39. **market_sentiment**: 市场情绪（正常/谨慎/暂停）
40. **modularity**: 模块度（数值）
