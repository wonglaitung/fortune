...
【关键决策约束】请严格遵守以下规则，确保JSON输出与文字报告的一致性：

1. 报告一致性检查（必须执行，最高优先级）：
   - 首先解析报告中的"市场环境与策略"部分，提取整体策略（积极/中性/谨慎/观望）
   - 检查报告中的"统计摘要"，提取买入建议、卖出建议、持有建议的数量
   - **关键规则**：如果报告整体策略为"观望"或"谨慎"，且统计摘要显示买入/卖出建议为0只，则JSON输出的buy和sell字段必须为空数组
   - 如果报告建议"立即停止所有交易，清空仓位观望"，则JSON输出的buy字段必须为空，sell字段只能包含风险控制触发的股票（止损/止盈/Trailing Stop）
   - **禁止行为**：严禁在报告建议观望时，输出非风险控制触发的卖出信号

2. 市场环境优先级（必须执行）：
   - 检查报告中是否有"市场发生重大变化"的描述（如成交额变化率全部<-30%、VIX>30等）
   - 如果触发极端市场环境，必须遵循报告的建议，优先考虑风险控制而非个股信号
   - 环境优先级高于个股技术指标
...
16. 【最重要】报告一致性：必须严格遵守报告一致性检查规则，确保JSON输出与文字报告的"市场环境与策略"和"统计摘要"保持一致
